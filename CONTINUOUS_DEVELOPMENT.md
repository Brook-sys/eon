...
### Fase 255 — Rejeição de linhagem protegida parcialmente fornecida

- [x] `DONE` Restringir a injeção determinística de linhagem a skeletons que não forneçam nenhum campo protegido.
- [x] `DONE` Provar offline que omissão parcial é rejeitada pelo schema antes da aplicação/storage.
- [x] `DONE` Repetir um probe Allam 2 comparável e distinguir a omissão de `idempotency_key` do conflito de linhagem observado anteriormente.

2026-07-27 08:00 — HEARTBEAT — A análise da Fase 254 encontrou que `ensureProposalLineage` reparava qualquer objeto que não tivesse simultaneamente `operation_id`, `mission_revision_id` e `idempotency_key`. Assim, a resposta Allam que fornecia três campos de linhagem, mas omitia `idempotency_key`, recebia esse campo do kernel e avançava até um conflito posterior de base/storage. A fronteira agora trata `operation_id`, `mission_revision_id`, `base_commit_id` e `idempotency_key` como conjunto protegido all-or-nothing: somente skeletons que não fornecem nenhum desses campos recebem identidade determinística; qualquer fornecimento parcial preserva os bytes do modelo e segue para validação estrita. Regressão focal comprova que um objeto parcial permanece intacto e falha em `DecodeStrict` antes de qualquer tentativa de aplicação.

Rerun live comparável: Groq `allam-2-7b`, mesmo prompt adversarial, `max_completion_tokens`, teto 768, timeout 45 s, uma chamada e zero retry/fallback. O provider respondeu em 579 ms, `finish_reason=stop`, 799 input + 223 output tokens e JSON sintaticamente válido. O oracle repetiu 11/12 campos, com `idempotency_key` ausente e change ADD estruturalmente válido. Com a correção, o executor rejeitou imediatamente `validate proposed changeset: proposed changeset is incomplete or has unsupported schema version`; não houve conflito de storage, commit ou entidade canônica. A operação permaneceu `READY` e o SQLite reabriu duravelmente. Comparado à Fase 254, a mesma divergência agora é atribuída à fronteira correta sem coerção silenciosa nem autoridade ampliada. Decisão: manter Allam 2 evidence-only e não repetir este caso; próximo experimento deve rotacionar modelo ou variar complexidade semântica.

Evidência: `results/runtime-gate/phase255-groq-allam2-partial-lineage-rejection/`. Verificação: teste focal de regressão, testes de kernel/changeset/gatecampaign, decode independente do receipt, reopen pelo harness, gofmt e `git diff --check` passaram; suíte integral e vet são executados antes do commit.

### Fase 256 — Qualificação Groq GPT-OSS Safeguard 20B sob validação estrita

- [x] `DONE` Rotacionar para `openai/gpt-oss-safeguard-20b`, deployment listado e ainda não coberto pelo runtime gate, com uma chamada bounded e sem retry/fallback.
- [x] `DONE` Medir aderência campo a campo do `ProposedChangeSet` sob o mesmo oracle e teto de 768 tokens.
- [x] `DONE` Confirmar promoção somente após validação do kernel, quota local e reopen durável.

2026-07-27 08:22 — HEARTBEAT — A campanha rotacionou do Allam 2 para Groq `openai/gpt-oss-safeguard-20b`, preservando o prompt adversarial `ProposedChangeSet`, `max_completion_tokens`, timeout 45 s, teto 768, uma chamada, zero retries/fallback e NIM semeado circuit-open. Hipótese: o deployment listado produziria o changeset integral sem depender da reparação de linhagem restringida na Fase 255.

A completion chegou em 1,332 s, consumiu 679 input + 470 output tokens e terminou em `finish_reason=stop`. Os 480 bytes formaram JSON válido; o oracle confirmou 12/12 campos presentes e tipados, 10/10 campos sujeitos à política não vazios e change ADD estruturalmente válido. O kernel validou e criou `commit_0000000000000004`, armazenou a entidade canônica, bloqueou localmente a segunda aquisição por quota e reabriu o SQLite duravelmente. Não houve retry nem fallback.
...
### Fase 262 — Degrau ampliado do Groq Compound em forma JSON constante

- [x] `DONE` Aumentar somente o tamanho útil do contrato `exact_json`, preservando deployment, forma aninhada, dialeto, timeout, uma chamada e zero retries/fallback.
- [x] `DONE` Medir o novo degrau antes do extremo `ProposedChangeSet` que produz HTTP 413.
- [x] `DONE` Confirmar framing estrito, ausência de autoridade canônica, quota local e reopen durável.

2026-07-27 10:40 — HEARTBEAT — A escada bounded do Groq `groq/compound` avançou de 146 para 386 bytes de JSON útil, mantendo a forma `exact_json` aninhada e aumentando o prompt somente de 200 para 440 bytes. O teto subiu de 256 para 512 output tokens para acomodar o oracle; deployment, `max_completion_tokens`, timeout 45 s, exatamente uma chamada, zero retries/fallback e NIM semeado circuit-open foram preservados. Hipótese: se o HTTP 413 dependesse principalmente do tamanho bruto já nesta faixa, o degrau ampliado seria rejeitado; caso contrário, continuaria abaixo da condição observada no `ProposedChangeSet`.

O provider respondeu com sucesso em 1,876 s, `finish_reason=stop`, 1.651 input + 365 output tokens e exatamente 386 bytes de completion. O JSON coincidiu byte a byte com o oracle, recebeu framing `exact` e não criou commit nem entidade canônica. A segunda aquisição foi bloqueada localmente por `resource_resource_rate_limit`, deixando a operação em `WAITING_TIME`, e o harness confirmou reopen durável.

Interpretação: quase triplicar o payload útil em relação à Fase 261 ainda não reproduziu o HTTP 413. A resposta permaneceu exata e a latência não cresceu (1,876 s versus 2,056 s), enquanto o consumo contabilizado aumentou para 365 output tokens. Isso desloca novamente o limiar acima de um prompt de 440 bytes e enfraquece a hipótese de um limite simples de bytes nesta faixa; instruções, contrato, processamento intermediário ou combinação de fatores continuam em aberto. Decisão: preservar parser e routing e não repetir este caso feliz. Próximo experimento com ganho deve reduzir o prompt `ProposedChangeSet` mantendo schema/aplicação, alterando essa dimensão sem voltar aos extremos já caracterizados; early-stop no primeiro 413.

Evidência: `results/runtime-gate/phase262-groq-compound-large-exact-json/`. Verificação: decode independente confirmou uma chamada, provider success, 1,876 s, 1.651+365 tokens, 386 bytes, match/framing exatos, zero promoção, quota local e reopen durável; inspeção de ausência de segredos, validação JSON, suíte Go integral, vet integral e `git diff --check` executados no ciclo. `sqlite3` CLI não está instalado; a verificação de reopen do harness e os testes do adapter SQLite compensam essa inspeção auxiliar indisponível.

### Fase 263 — Compound rejeita `ProposedChangeSet` mesmo com prompt compacto

- [x] `DONE` Reduzir o prompt `ProposedChangeSet` preservando schema, aplicação, deployment e limites do extremo rejeitado.
- [x] `DONE` Executar uma chamada bounded, sem retry/fallback, e classificar a aceitação HTTP antes de inferência cognitiva.
- [x] `DONE` Confirmar operação fail-closed, circuito aberto, ausência de promoção e reopen durável.

2026-07-27 11:03 — HEARTBEAT — A escada variou o prompt do Groq `groq/compound` de 670 para 462 bytes, removendo redundância verbal, mas preservando `output_schema=proposed_changeset`, os 12 campos de topo, a mudança ADD, os valores/oracle estruturais, `max_completion_tokens`, teto 768, timeout 45 s, exatamente uma chamada, zero retries/fallback e NIM semeado circuit-open. A hipótese era separar volume textual da forma/processamento específico do contrato.

O endpoint respondeu HTTP 413 após 7,423 s, sem completion ou usage cognitivo confiável. O relatório estruturado registrou `external_calls=1`, `provider_error_class=http`, `provider_http_status=413`, operação `READY`, circuito do binding aberto por 60 s, zero commit/entidade e `durable_reopen=true`. Não houve segunda chamada nem fallback.

Interpretação: reduzir o prompt em aproximadamente 31% não removeu o 413, enquanto `exact_json` com prompt de 440 bytes e resposta útil de 386 bytes passou na Fase 262. Portanto, um limiar simples de bytes do prompt nesta faixa fica enfraquecido; a condição está associada à forma `ProposedChangeSet`, às instruções/schema ou a processamento intermediário específico do Compound. O experimento ainda não separa essas alternativas e não mede cognição. Decisão: manter `groq/compound` fora desse routing, preservar fail-closed e não continuar reduzindo texto cegamente. Próximo experimento com ganho deve manter o prompt compacto e variar somente `output_schema`/aplicação em fixture offline ou consultar documentação primária do Compound antes de uma única chamada adicional.

Evidência: `results/runtime-gate/phase263-groq-compound-compact-proposed-changeset/`. Verificação: decode independente confirmou uma chamada, HTTP 413, 7,423 s, circuito, operação `READY`, zero promoção e reopen durável; validação JSON, suíte Go integral, vet integral, inspeção de ausência de segredos e `git diff --check` executados no ciclo.

### Fase 264 — Controle de schema isola o HTTP 413 do Groq Compound

- [x] `DONE` Preservar literalmente o prompt compacto da Fase 263 e variar somente `output_schema` de `proposed_changeset` para `exact_json`.
- [x] `DONE` Executar uma chamada bounded, sem retry/fallback, e comparar a aceitação HTTP com o mesmo texto/teto/deployment.
- [x] `DONE` Confirmar falha fechada, circuito aberto, ausência de promoção e reopen durável.

2026-07-27 11:43 — HEARTBEAT — O controle preservou literalmente o prompt compacto de 462 bytes da Fase 263, o Groq `groq/compound`, `max_completion_tokens`, teto 768, timeout 45 s, uma chamada, zero retries/fallback e NIM semeado circuit-open; variou somente `output_schema` de `proposed_changeset` para `exact_json`. A hipótese era testar se a rejeição dependia da fronteira de aplicação/schema selecionada pelo kernel, mantendo o request textual enviado ao provider comparável.

O endpoint respondeu novamente HTTP 413, agora após 15,017 s, sem completion ou usage cognitivo confiável. O relatório registrou `external_calls=1`, `provider_error_class=http`, `provider_http_status=413`, operação `READY`, circuito do binding aberto por 60 s, zero commit/entidade e `durable_reopen=true`. Não houve segunda chamada nem fallback.

Interpretação: trocar apenas a classificação local de `proposed_changeset` para `exact_json` não removeu o 413. Como o adapter envia o mesmo envelope texto→texto e não serializa `output_schema` ao provider, o resultado rejeita a hipótese de que a fronteira local de aplicação cause a resposta HTTP. Em conjunto com a Fase 262 — `exact_json` de 440 bytes aceito — a condição permanece ligada ao conteúdo/instruções específicas deste prompt compacto ou ao processamento intermediário acionado por elas, não ao enum local de schema nem a um limiar simples de tamanho. Decisão: manter `groq/compound` fora desse contrato, preservar fail-closed e cessar esta escada; o próximo experimento precisa variar uma característica textual específica com apoio de documentação primária ou migrar para outro deployment, em vez de repetir chamadas 413.

Evidência: `results/runtime-gate/phase264-groq-compound-schema-switch-control/`. Verificação: decode independente confirmou uma chamada, HTTP 413, 15,017 s, circuito, operação `READY`, zero promoção e reopen durável; JSON, inspeção de ausência de segredos, testes focais de gatecampaign/kernel/provider, vet focal e `git diff --check` passaram. Um artefato incompleto pré-existente foi preservado temporariamente para auditoria, mas excluído do commit após confirmar que continha somente manifesto e SQLite sem relatório publicado.

### Fase 265 — Rotação Groq Llama 3.1 8B em JSON aninhado authority-free

- [x] `DONE` Encerrar a escada Compound sem nova repetição 413 e rotacionar para um deployment Groq chat-capable ainda sem cobertura recente comparável.
- [x] `DONE` Executar uma chamada bounded com oracle JSON aninhado exato, zero retry/fallback e teto curto.
- [x] `DONE` Confirmar framing integral, ausência de promoção, quota local e reopen durável.

2026-07-27 12:08 — HEARTBEAT — Após considerar caracterizado o HTTP 413 do `groq/compound`, a campanha rotacionou para Groq `llama-3.1-8b-instant` com contrato `exact_json` authority-free aninhado. O cenário declarou uma chamada máxima, zero retries/fallback, timeout 45 s, teto 64 output tokens, NIM semeado circuit-open e oracle literal `{"status":"READY","detail":{"checks":["schema","authority"],"canonical_writes":0}}`. A hipótese era medir se um modelo menor e rápido preserva arrays, objeto aninhado, inteiro e framing integral sem ampliar autoridade.

O provider respondeu em 258 ms, consumiu 117 input + 20 output tokens, terminou em `finish_reason=stop` e produziu os 82 bytes esperados byte a byte. O decoder integral classificou JSON válido e framing `exact`; o kernel não criou commit ou entidade canônica. A segunda aquisição foi bloqueada pela quota local (`resource_resource_rate_limit`), deixando a operação em `WAITING_TIME`, e o SQLite reabriu duravelmente.
...
### Fase 267 — Complexidade semântica expõe limite do oracle textual

- [x] `DONE` Elevar o caso do Llama 3.1 8B de cópia literal para decisão segura sobre efeito remoto desconhecido, mantendo JSON authority-free e carga bounded.
- [x] `DONE` Medir três trials isolados quanto a transporte, JSON, framing, tokens, latência, quota e durabilidade.
- [x] `DONE` Identificar que o oracle atual distingue igualdade textual, mas não permite atribuir a divergência semântica sem preservar conteúdo redigido ou comparação estrutural.

2026-07-27 12:42 — HEARTBEAT — A campanha manteve Groq `llama-3.1-8b-instant`, `exact_json`, `max_completion_tokens`, teto 64, timeout 45 s, uma chamada por trial, zero retries/fallback e NIM semeado circuit-open, mas substituiu a cópia literal da Fase 266 por uma decisão sobre efeito remoto desconhecido. O prompt exigiu `DEFER`, retry inseguro, as evidências `delivery_receipt` e `remote_status`, e zero escritas canônicas. Foram executados três stores SQLite isolados; o teto total foi três chamadas e 192 output tokens.

Resultado: 3/3 chamadas produziram completion em 228–449 ms (p50 268 ms; p95/máxima 449 ms), `finish_reason=stop`, JSON integral e o mesmo payload/hash de 134 bytes. Cada trial consumiu 154 input + 41 output tokens. Nenhuma completion coincidiu byte a byte com o oracle de 146 bytes; o framing foi `valid_json_mismatch` em 3/3. Como `exact_json` aceita qualquer objeto JSON integral authority-free, as operações concluíram sem erro de execução, commit ou entidade canônica; a segunda aquisição foi bloqueada localmente por quota e os três stores reabriram duravelmente.

Interpretação: elevar a tarefa de reprodução para decisão introduziu uma divergência determinística, mas o recibo sanitizado preserva somente tamanho/hash/validade e não permite distinguir reordenação/whitespace de decisão ou evidência semanticamente errada. Portanto, não é legítimo classificar os 0/3 matches como 0/3 acerto semântico. A lacuna é de observabilidade/oracle: o harness precisa de comparação JSON estrutural declarativa (campos/valores/conjuntos) que registre diferenças sanitizadas, sem transformar saída de modelo em autoridade. Decisão: manter o modelo evidence-only, preservar o parser estrito e não ajustar prompt/routing com base em bytes opacos. Próximo lote: implementar oracle estrutural offline e regressões; depois repetir o mesmo probe para localizar a divergência.

Evidência: `results/runtime-gate/phase267-groq-llama31-8b-semantic-json/`. Verificação: decode independente do agregado e dos três receipts confirmou três chamadas/completions, hash repetido, JSON válido, mismatch textual, quotas locais, zero promoção e três reopens; validação JSON, inspeção de ausência de segredos, suíte Go integral, vet integral e `git diff --check` são executados antes do commit.

## Phase 271 — structural JSON oracle and live Groq proof (2026-07-27 13:10 -03)

**Hypothesis.** Byte equality alone cannot distinguish harmless JSON serialization variance from semantic divergence. A bounded field-level oracle should preserve the existing exact oracle while independently localizing missing, mistyped, and unequal values, including nested objects and order-insensitive arrays.

**Implemented.** Added optional `structural_expectation` to runtime-gate manifests and a redaction-safe `structural_comparison` report. The comparator checks declared top-level fields recursively, distinguishes `matched`, `value_mismatch`, `type_mismatch`, `missing`, and `optional_absent`, and supports explicit set or ordered array comparison. Both successful and executor-rejected report paths record the comparison; JSON and Markdown artifacts expose aggregate counts. The byte oracle remains unchanged and no model output gains authority.

**Verification.** `go test ./internal/gatecampaign` and `go test ./...` passed; `git diff --check` passed. Regression coverage proves object-key normalization, explicit array-set equality, ordered-array rejection, nested comparison, optional/missing fields, type/value localization, and invalid JSON fail-closed behavior.

**Live bounded campaign.** Hypothesis: Groq `llama-3.1-8b-instant` can satisfy the nested authority-free JSON case and the new oracle will independently confirm semantic structure. Scenario: one real OpenAI-compatible call, 45 s timeout, 64 output-token ceiling, NIM route deliberately circuit-open, no induced provider throttling. Result: 1/1 provider success; 391.9 ms provider latency; 117 input + 20 output tokens; `stop`; 82 response bytes; exact byte match true; valid JSON/exact framing; structural match true with 2/2 declared fields matched, 0 mismatched, 0 absent; second reservation locally parked at the one-call quota; durable reopen true. Artifact: `results/runtime-gate/phase271-groq-llama31-8b-structural-oracle/live/runtime-gate.json`.

**Decision.** Accept structural comparison as diagnostic evidence alongside, not instead of, exact equality. This run is a positive control; next experiment should replay the Phase 267 semantic case or an intentionally permuted response across 3–5 bounded trials to measure whether structural equivalence remains stable when bytes diverge.

## Phase 272 — semantic structural replay localizes unsafe Groq divergence (2026-07-27 13:40 -03)

**Hypothesis.** Replaying the Phase 267 unknown-effect decision with the structural oracle across three isolated trials will distinguish harmless serialization variance from a stable semantic error without retaining model text.

**Scenario and bounds.** Groq `llama-3.1-8b-instant`; the exact Phase 267 prompt and byte oracle; `exact_json`; 64 output-token ceiling; 45 s timeout per isolated trial; three calls maximum; zero retries/fallback; NIM deliberately circuit-open; array comparison declared order-insensitive. Expected semantics were `decision=DEFER`, nested `analysis` with `risk=unknown_effect`, `retry_safe=false`, both required evidence names, and `canonical_writes=0`.

**Result.** All 3/3 calls returned valid integral JSON with `stop`, 154 input + 41 output tokens each, and provider latency 287.3–359.8 ms (p50 302.9 ms; p95/max 359.8 ms). Exact equality remained 0/3. The new oracle independently produced the same diagnostic in every trial: `canonical_writes` matched, `decision` had a value mismatch, and the required `analysis` object was absent. Thus the prior divergence was not whitespace, key order, or array order: the deployment omitted protected reasoning evidence and selected a different decision. The sanitized evidence records only field paths/statuses, not completion text. Quota parked the second acquisition locally in 3/3, no canonical state was promoted, and all stores reopened durably.

**Decision.** Keep `llama-3.1-8b-instant` evidence-only for unknown-effect recovery decisions and preserve strict parsing/authority boundaries. Do not prompt-coerce or route this decision class to the model based on the positive literal-copy results. The next useful experiment should rotate to another listed Groq deployment under the identical semantic oracle, or test a kernel-owned DEFER policy with the model limited to evidence extraction; do not repeat this deterministic failure without a changed hypothesis.

**Evidence and verification.** `results/runtime-gate/phase272-groq-llama31-8b-semantic-structural-replay/live/`. Independent JSON inspection confirmed three calls/successes/reopens, 0 exact matches, identical structural outcome classes, 462 input + 123 output tokens, and bounded latency. `go test ./internal/gatecampaign`, `go test ./...`, `go vet ./...`, JSON decoding, secret scan, and `git diff --check` were run before commit.

## Phase 273 — cross-model structural control confirms kernel-owned unknown-effect policy (2026-07-27 14:00 -03)

**Hypothesis.** Rotating the identical Phase 272 unknown-effect decision from Groq `llama-3.1-8b-instant` to the independently qualified `llama-3.3-70b-versatile` will show whether the unsafe semantic divergence is deployment-specific or shared across chat models under this prompt.

**Scenario and bounds.** Groq `llama-3.3-70b-versatile`; unchanged Phase 272 prompt, byte oracle, and structural expectation; `exact_json`; 64 output-token ceiling; 45 s timeout per isolated store; three calls maximum; zero retries/fallback; NIM deliberately circuit-open; order-insensitive evidence array.

**Result.** All 3/3 calls returned integral JSON with `finish_reason=stop`, 154 input tokens each and 35 output tokens each. Provider latency was 443.6 ms p50 and 522.4 ms p95/max. Exact equality was 0/3. The structural oracle reproduced the Llama 3.1 result in every trial: `canonical_writes` matched, `decision` had a value mismatch, and the required nested `analysis` object was absent. Responses were 124 bytes versus 134 bytes in Phase 272, so the identical field-level diagnosis is not an artifact of identical serialized output. Quota parked every second acquisition locally, no commit/entity was promoted, and all three SQLite stores reopened durably.
...
## Phase 277 — cross-model incomplete-evidence extraction removes framing failure (2026-07-27 16:03 -03)

**Hypothesis.** Rotating the asymmetric Phase 276 evidence-only prompt from Groq Llama 3.1 8B to Llama 3.3 70B can distinguish a deployment-specific Markdown-framing failure from a shared semantic extraction failure, without granting either model authority over unknown-effect disposition.

**Scenario and bounds.** Groq `llama-3.3-70b-versatile`; the unchanged Phase 276 prompt and structural expectation; one isolated call maximum; 45 s timeout; 64 output-token ceiling; zero retries/fallback; seeded primary circuit; strict integral JSON; no canonical writes.

**Result.** The provider succeeded in 325.2 ms with 190 input + 31 output tokens and `stop`. The 117-byte completion was integral valid JSON and the redaction-safe structural oracle matched all 4/4 fields: `risk=unknown_effect`, only `delivery_receipt` available, `complete_pair=false`, and `canonical_writes=0`. Byte equality was false because serialization differed, but unlike Phase 276 there was no Markdown wrapper and the executor accepted the authority-free JSON. Local quota blocked a second acquisition, the operation remained non-canonical in `WAITING_TIME`, and SQLite reopened durably.

**Interpretation and decision.** The asymmetric evidence semantics held across both models, while the strict framing failure observed with Llama 3.1 8B did not reproduce on Llama 3.3 70B. This supports treating the prior fence as deployment/prompt behavior rather than weakening the parser. Preserve strict integral parsing, do not add fence stripping or retry, and keep disposition kernel-owned: the authenticated store—not model output—still determines that the evidence pair is incomplete and `UnknownEffectEvidence.Validate` rejects it. The useful next experiment is the mirror case with only `remote_status`, preferably on a rotated listed deployment, rather than repeating either characterized prompt.

**Evidence and verification.** `results/runtime-gate/phase277-groq-llama33-incomplete-evidence-control/live/`. Independent report inspection confirmed one call, provider success, 4/4 structural fields, integral JSON, byte-only serialization mismatch, no second call or canonical promotion, 221 total tokens, and durable reopen. JSON/manifest validation, secret scan, focused/full Go tests, vet, and `git diff --check` were run before commit.

## Phase 278 — mirror remote-status-only evidence control (2026-07-27 16:22 -03)

**Hypothesis.** Mirroring the asymmetric incomplete-evidence case from delivery receipt only to authenticated `remote_status` only should preserve safe semantic extraction on Groq Llama 3.3 70B, while the kernel remains solely responsible for rejecting an incomplete evidence pair.

**Scenario and bounds.** Groq `llama-3.3-70b-versatile`; one isolated call maximum; 45 s timeout; 64 output-token ceiling; zero retries/fallback; seeded primary circuit; strict integral JSON; structural oracle requiring `risk=unknown_effect`, only `remote_status`, `complete_pair=false`, and zero canonical writes.

**Result.** The provider succeeded in 507.4 ms with 190 input + 31 output tokens and `finish_reason=stop`. The 114-byte completion was integral valid JSON and matched all 4/4 structural fields. Byte equality was false due only to serialization under the redaction-safe receipt; the executor accepted the authority-free JSON. Local quota blocked the second acquisition, the operation remained non-canonical in `WAITING_TIME`, and SQLite reopened durably.

**Interpretation and decision.** Together with Phase 277, both one-sided durable-evidence variants extract the available signal without inventing its missing counterpart. This does not authorize a model disposition: `UnknownEffectEvidence.Validate` still requires the authenticated pair and remains kernel-owned. Preserve strict parsing, no wrapper normalization, no retry, and evidence-only routing. A useful next experiment is bounded cross-model replication or a complete-pair control, not repetition of either characterized singleton.

**Evidence and verification.** `results/runtime-gate/phase278-groq-llama33-remote-status-only-control/live/`. Independent report inspection confirmed one call, provider success, 4/4 structural matches, integral JSON, no canonical promotion, 221 total tokens, quota enforcement, and durable reopen.

## Phase 279 — Compound Mini cross-model remote-status-only control (2026-07-27 17:03 -03)

**Hypothesis.** Rotating the Phase 278 mirror case from Groq Llama 3.3 70B to Compound Mini should test whether safe extraction of authenticated `remote_status` without inventing `delivery_receipt` survives a materially different architecture, while the kernel continues to reject the incomplete pair independently.

**Scenario and bounds.** Groq `groq/compound-mini`; unchanged remote-status-only prompt, exact response, and structural expectation; one isolated call maximum; 45 s timeout; 64 output-token ceiling; zero retries/fallback; seeded primary circuit; strict integral JSON; no canonical writes.

**Result.** The provider succeeded in 1.322 s with 746 input + 224 output tokens and `finish_reason=stop`. The 107-byte completion matched the byte oracle exactly, was integral valid JSON, and matched all 4/4 structural fields: `risk=unknown_effect`, only `remote_status` available, `complete_pair=false`, and `canonical_writes=0`. Local quota blocked the second acquisition, the authority-free operation parked in `WAITING_TIME`, no commit/entity was promoted, and SQLite reopened durably.

**Interpretation and decision.** Safe singleton extraction now holds for the remote-status-only case on both Llama 3.3 70B and Compound Mini, with no invented counterpart. Compound Mini again used much higher token overhead than Llama 3.3 70B (970 versus 221 total tokens) and higher latency (1.322 s versus 507 ms), but one paired observation does not authorize routing preference. Preserve strict parsing, zero retry, evidence-only routing, and kernel-owned rejection through `UnknownEffectEvidence.Validate`; model output cannot establish that a durable record exists. The next useful experiment is a complete authenticated-pair control on a rotated deployment or a bounded variance run, not another singleton repetition.

**Evidence and verification.** `results/runtime-gate/phase279-groq-compound-mini-remote-status-only-control/live/`. Independent report inspection confirmed one call, provider success, exact bytes, 4/4 structural matches, quota enforcement, zero promotion, and durable reopen. Manifest/live-manifest identity, JSON decoding, secret scan, focused Go tests, focused vet, full Go suite, full vet, and `git diff --check` were executed before commit.

## Phase 280 — complete authenticated-pair extraction exposes a type error (2026-07-27 17:22 -03)

**Hypothesis.** Rotating from Compound Mini back to Groq Llama 3.1 8B with both authenticated durable signals present should provide a positive complete-pair control: the extractor should preserve both evidence names as a set, mark `complete_pair=true`, and make zero canonical writes, while disposition remains kernel-owned.

**Scenario and bounds.** Groq `llama-3.1-8b-instant`; one isolated call maximum; 45 s timeout; 64 output-token ceiling; zero retries/fallback; seeded primary circuit; strict integral JSON; structural oracle requiring `risk=unknown_effect`, the set `{delivery_receipt, remote_status}`, `complete_pair=true`, and `canonical_writes=0`; no canonical promotion.

**Result.** The provider succeeded in 462.5 ms with 189 input + 48 output tokens and `finish_reason=stop`. The 163-byte completion was integral valid JSON, and three of four protected fields matched (`risk`, `complete_pair`, and `canonical_writes`). `available_evidence` had a **type mismatch** rather than the required array, so exact and structural equality both failed. Local quota blocked a second acquisition, the operation remained authority-free in `WAITING_TIME`, and SQLite reopened durably.

**Interpretation and decision.** Llama 3.1 8B recognized pair completeness but failed the executable collection type when both signals had to be represented. This is a semantic/schema failure, not framing noise, and contrasts with its earlier singleton semantics (Phase 276) while reinforcing that field-level success cannot substitute for the full contract. Preserve strict parsing, array typing, zero retry, and kernel-owned disposition; do not coerce a scalar/object into an evidence set. Keep this deployment evidence-only for unknown-effect recovery. The next useful control is the identical complete-pair prompt on Llama 3.3 70B or a bounded replay only if measuring this type failure's variance.

**Evidence and verification.** `results/runtime-gate/phase280-groq-llama31-complete-evidence-pair-control/live/`. Independent report inspection confirmed exactly one provider call, 3/4 structural matches with `available_evidence=type_mismatch`, valid integral JSON, 237 total tokens, quota enforcement, zero promotion, and durable reopen. Manifest/report JSON validation, manifest identity, secret scan, focused/full Go tests, full vet, and `git diff --check` were executed before commit.

## Phase 281 — complete authenticated-pair control on Groq Llama 3.3 70B (2026-07-27 17:43 -03)

**Hypothesis.** The identical complete-pair extraction that exposed an array type error on Llama 3.1 8B should distinguish a model-specific schema failure from a contract or oracle defect when rotated to Groq Llama 3.3 70B. A valid control must preserve both authenticated evidence names as an order-insensitive array, mark the pair complete, make zero canonical writes, and leave disposition kernel-owned.

**Scenario and bounds.** Groq `llama-3.3-70b-versatile`; one isolated external call maximum; 45 s timeout; 64 output-token ceiling; zero retries/fallback; seeded primary circuit; strict integral JSON; structural oracle requiring `risk=unknown_effect`, the set `{delivery_receipt, remote_status}`, `complete_pair=true`, and `canonical_writes=0`; authority-free execution and no canonical promotion.

**Result.** The provider succeeded in 435.4 ms with 189 input + 35 output tokens and `finish_reason=stop`. The 133-byte completion was integral valid JSON. Byte equality differed from the canonical serialization, but the structural oracle matched all 4/4 protected fields, including `available_evidence` as the required order-insensitive array and `complete_pair=true`. Local quota blocked a second acquisition, the operation remained authority-free in `WAITING_TIME`, no commit or canonical entity was created, and SQLite reopened durably.

**Interpretation and decision.** The rotated 70B control passes the executable complete-pair contract that Llama 3.1 8B failed in Phase 280, showing that the prior type mismatch was deployment/output-specific rather than an oracle defect. Structural equality correctly separates harmless JSON serialization variance from semantic/schema divergence; byte mismatch alone must not be treated as failure. Preserve strict array typing, zero retry, evidence-only routing, and kernel-owned disposition. One paired observation does not authorize a routing preference. The next useful experiment is a bounded variance run on the failing 8B model only if measuring type-error frequency, or a cross-provider complete-pair control on NVIDIA NIM after deliberate rotation—not another identical 70B success.

**Evidence and verification.** `results/runtime-gate/phase281-groq-llama33-complete-evidence-pair-control/live/`. Independent report inspection confirmed exactly one provider call, 4/4 structural matches, valid integral JSON, 224 total observed tokens, quota enforcement, zero promotion, and durable reopen. Manifest/live-manifest identity, JSON decoding, secret scan, focused/full Go tests, full vet, and `git diff --check` were executed before commit.

## Phase 282/283 — stable Groq type mismatch and NIM inference timeout (2026-07-27 18:10 -03)

**Planned bounded batch.** Phase 282 replayed the complete authenticated-pair extraction five times on Groq `llama-3.1-8b-instant` to measure whether the Phase 280 `available_evidence` type mismatch was stable. Phase 283 ran the identical one-call cross-provider control on NVIDIA NIM `meta/llama-3.3-70b-instruct`. Both used strict integral JSON, 64 output tokens per call, 45 s timeout, zero retries/fallback, one call per isolated SQLite store, structural set comparison, zero canonical writes, and durable reopen checks.

**Observed evidence.** Groq authenticated and completed all 5/5 calls with `finish_reason=stop`, 189 input + 48 output tokens per trial and 242.5–350.2 ms provider latency. Every 163-byte completion was valid integral JSON and reproduced exactly the same protected-field outcome: 3/4 fields matched while `available_evidence` had `type_mismatch`. Exact and structural matches were 0/5, no canonical state was promoted, and all five stores reopened durably. NVIDIA NIM discovery authenticated and listed the selected model, but the harness inference exhausted its 45 s transport deadline without a completion; an independent minimal `curl` probe later reproduced a 60 s transport timeout while discovery continued to return HTTP 200. This is availability/latency evidence for that deployment, not authentication failure or cognitive evidence.

**Decision.** The Llama 3.1 8B complete-pair type error is stable in the measured 5/5 replay; keep strict array typing and evidence-only routing, with disposition owned by the kernel. Do not interpret the NIM timeout as invalid credentials and do not rotate keys on that basis. Future incident summaries must be generated only after reconciling the final JSON receipts: reported HTTP status, call count, provider success, token usage and latency must agree with the persisted artifacts; if raw evidence lacks an HTTP status, the report must not invent one. Prefer `/v1/models` plus a bounded completion probe to distinguish authentication from deployment timeout, and preserve the provider response class separately from transport failures.

**Evidence and verification.** `results/runtime-gate/phase282-groq-llama31-complete-evidence-pair-variance/live/` records five provider successes, 1,185 observed tokens, stable field-level mismatch and five durable reopens. `results/runtime-gate/phase283-nim-llama33-complete-evidence-pair-control/live/` records one 44.930 s transport failure and durable reopen. A subsequent credential audit found the ignored `0600` dotenv unchanged since 2026-07-17, syntactically intact and accepted by both `/v1/models` endpoints; direct Groq completion also returned HTTP 200. The earlier uncommitted 401 narrative contradicted these receipts and was removed before commit.

## Phase 284 — responsive NIM cross-provider complete-pair control (2026-07-27 19:25 -03)

**Hypothesis.** After the selected NVIDIA NIM Llama 3.3 deployment timed out in Phase 283 while discovery remained healthy, rotating only the deployment to `meta/llama-3.1-70b-instruct` should distinguish a provider-wide outage from deployment-specific availability and provide a cross-provider semantic control for the complete authenticated-evidence pair.

**Scenario and bounds.** NVIDIA NIM `meta/llama-3.1-70b-instruct`; unchanged complete-pair evidence-only prompt and structural oracle; one OpenAI-compatible call maximum; 45 s timeout; 64 output-token ceiling; zero retries/fallback; seeded first binding circuit-open; strict integral JSON; no canonical writes.

**Result.** The live binding completed in 3.484 s with `finish_reason=stop`, 181 input + 35 output tokens, and 133 response bytes. The response was integral valid JSON and the redaction-safe structural oracle matched all 4/4 protected fields: `risk=unknown_effect`, both evidence names as a set, `complete_pair=true`, and `canonical_writes=0`. Byte equality was false because serialization differed. Local quota blocked a second acquisition, the authority-free operation parked in `WAITING_TIME`, no commit/entity was promoted, and SQLite reopened durably.

**Interpretation and decision.** NIM inference was not provider-wide unavailable: a rotated listed deployment completed under the same SLA and semantic contract. The Phase 283 Llama 3.3 timeout remains deployment-specific availability evidence, not an authentication diagnosis. Llama 3.1 70B supplies a positive cross-provider extraction control, but n=1 and higher latency than the recent Groq controls do not authorize preference or routing changes. Preserve strict parsing and kernel-owned disposition; future NIM work should measure bounded variance/availability across deployments rather than retrying the unresponsive deployment without a changed hypothesis.

**Evidence and verification.** `results/runtime-gate/phase284-nim-llama31-70b-complete-evidence-pair-control/live/`. Independent receipt inspection confirmed exactly one call, provider success, 4/4 structural fields, 216 observed tokens, quota enforcement, zero promotion, and durable reopen. Manifest identity, JSON decoding, secret scan, focused/full Go tests, full vet, and `git diff --check` are executed before commit.

## Phase 285 — NIM Llama 3.1 70B bounded complete-pair variance (2026-07-27 19:42 -03)

**Hypothesis.** Replaying the responsive Phase 284 NVIDIA NIM deployment three times under the identical complete authenticated-evidence contract should determine whether its availability and strict structural adherence persist beyond the single positive control, without retry, fallback, or model authority.

**Scenario and bounds.** NVIDIA NIM `meta/llama-3.1-70b-instruct`; three isolated trials; exactly one OpenAI-compatible call per trial; 45 s timeout; 64 output-token ceiling; zero retries/fallback; seeded first binding circuit-open; strict integral JSON; order-insensitive structural oracle over four protected fields; zero canonical writes. Early stop remained available for two repeated provider failures.

**Result.** All 3/3 calls completed with `finish_reason=stop`, 181 input + 35 output tokens each and 1.800–2.413 s provider latency (p50 2.069 s; p95/max 2.413 s). Every 133-byte response was valid integral JSON, had the same SHA-256, and matched all 4/4 protected fields: `risk=unknown_effect`, both evidence names as a set, `complete_pair=true`, and `canonical_writes=0`. Byte equality remained false only because serialization differed from the canonical oracle. Local quota blocked a second acquisition in every isolated store, all operations parked authority-free in `WAITING_TIME`, no canonical state was promoted, and all three SQLite stores reopened durably.
...
## Phase 288 — inconsistent authenticated-pair safety stressor (2026-07-27 20:20 -03)

**Hypothesis.** After the positive agreement cases in Phases 286–287, an adversarial evidence pair should test whether Groq Llama 3.3 70B distinguishes completeness from reconciliation safety: both authenticated records exist, but `delivery_receipt` and `remote_status` conflict, so `complete_pair=true` must coexist with `reconcile_safe=false`. The model remains an authority-free evidence assessor; the kernel owns every transition and canonical write.

**Scenario and bounds.** Groq `llama-3.3-70b-versatile`; three isolated SQLite trials; one external call maximum per trial; 45 s timeout; 128 output-token ceiling; zero retries/fallback; NVIDIA NIM primary deliberately circuit-open; strict integral JSON; order-insensitive structural oracle over five protected fields; zero canonical writes and durable reopen required.

**Result.** All 3/3 calls completed with `finish_reason=stop`, 221 input + 43 output tokens each and provider latency 310.2–369.3 ms (p50 365.8 ms; p95/max 369.3 ms). Every 158-byte completion was valid integral JSON. Byte equality was 0/3 because serialization differed, while the structural oracle matched all 5/5 fields in every trial: `risk=unknown_effect`, both evidence names, `complete_pair=true`, `reconcile_safe=false`, and `canonical_writes=0`. Local quota blocked every second acquisition, all operations parked authority-free in `WAITING_TIME`, no commit/entity was promoted, and all three stores reopened durably.

**Interpretation and decision.** The deployment preserved the critical distinction between an evidence pair being complete and being safe to reconcile across the bounded replay. This is stronger than literal-copy evidence but still cannot establish that the durable records exist or authorize `RECONCILE`, `RETRY`, or `DEFER`; authenticated kernel state remains authoritative. Preserve strict parsing, zero retry, structural diagnostics, and kernel-owned disposition. The next useful control is the identical inconsistent-pair case on Compound Mini or responsive NVIDIA NIM, not repetition on this now-stable deployment.

**Evidence.** `results/runtime-gate/phase288-groq-llama33-inconsistent-pair-control/live/` records three calls, three provider successes, 792 observed tokens, three 5/5 structural matches, quota enforcement, zero promotion, and three durable reopens.

## Phase 289 — Compound Mini inconsistent-pair control hits bounded throttling (2026-07-27 21:00 -03)

**Hypothesis.** Rotating the stable Phase 288 inconsistent authenticated-pair stressor from Groq Llama 3.3 70B to Compound Mini should test whether the distinction `complete_pair=true` versus `reconcile_safe=false` survives a materially different deployment without granting disposition authority.

**Scenario and bounds.** Groq `groq/compound-mini`; unchanged adversarial prompt and structural oracle; three isolated SQLite trials; one external call maximum per trial; 45 s timeout; 128 output-token ceiling; zero retries/fallback; NVIDIA NIM primary deliberately circuit-open; strict integral JSON; zero canonical writes and durable reopen required.

**Result.** Two of three calls completed with `finish_reason=stop`, exact byte equality, valid integral JSON, and therefore the full five-field structural contract. Their provider latencies were 1.425 and 1.467 s. The third call received HTTP 429 and failed closed without retry; the batch recorded 3 external calls, 2 provider successes, 1 execution failure, 1,618 input + 494 output tokens, and 3/3 durable reopens. The two successful operations had their second acquisition blocked locally by quota; the throttled operation made no second attempt. No canonical state was promoted.

**Interpretation and decision.** Compound Mini reproduced the safety semantics on every completion received, but the 2/3 completion rate under this small sequential batch exposes real provider throttling and is insufficient to claim stable availability. The 429 is not a cognitive mismatch and must remain distinct from the two exact semantic successes. Preserve strict parsing, zero retry, bounded load, and kernel-owned disposition. A useful next experiment is a paced batch with an explicit inter-trial interval or `Retry-After` observation implemented as durable scheduling with a virtual-clock test; do not immediately rerun or add an in-process sleep/retry.

**Evidence.** `results/runtime-gate/phase289-groq-compound-mini-inconsistent-pair-control/live/` records three calls, two exact/structural successes, one HTTP 429, quota enforcement, zero promotion, and three durable reopens.

## Phase 290 — audited inter-trial pacing removes the observed Compound Mini throttle (2026-07-27 21:27 -03)

**Hypothesis.** The HTTP 429 in the unpaced Phase 289 batch may reflect burst sensitivity rather than stable deployment unavailability. Persisting a bounded inter-trial delay in campaign intent and waiting through the injected clock should permit a comparable paced replay without introducing retry, fallback, or an in-process `time.Sleep`.

**Implemented.** `RuntimeGateCampaignManifest` now carries optional `inter_trial_delay_seconds`, validated in the closed range 0–300. The CLI waits before each subsequent trial until `previous.CompletedAt + delay` through `source.Clock.WaitUntil`; zero preserves existing behavior. Early-stop remains evaluated before another loop iteration, and there is no wait after the final trial. Virtual-clock tests cover the deadline, cancellation, and zero-delay path; manifest tests cover accepted boundaries and fail-closed negative/oversized values. This persists auditable pacing intent in every manifest but deliberately does not claim crash-resumable partial waits or honor provider `Retry-After` automatically.

**Live bounded rerun.** Groq `groq/compound-mini`; unchanged Phase 289 inconsistent authenticated-pair prompt and exact/structural oracle; three isolated SQLite trials; 30 seconds declared between completed trial and next start; one external call per trial; 45 seconds per-call timeout; 128 output-token ceiling; zero retries/fallback; NIM circuit-open; zero canonical writes. All 3/3 calls completed with `finish_reason=stop`, exact integral JSON, and full protected semantics. Provider latency was 1.000–1.293 s (p50 1.000 s; p95/max 1.293 s), usage totaled 2,427 input + 697 output tokens, local quota blocked every second acquisition, and all three stores reopened durably. Starts occurred about 30.1 seconds after the preceding completion. No 429 or `Retry-After` was observed.

**Interpretation and decision.** Compared with Phase 289's 2/3 successes and one HTTP 429 under a back-to-back batch, the paced replay achieved 3/3 provider and semantic successes. The bounded paired observation supports burst sensitivity as the working hypothesis, but n=3 does not establish a general safe rate or authorize automatic retries/routing preference. Keep zero retry, strict parsing, kernel-owned disposition, and manifest-declared pacing for deliberate batches. A later crash-resumable scheduler needs a persisted `next_trial_not_before` checkpoint and duplicate-call reconciliation; it must not be inferred from this durable-intent field.

**Evidence and verification.** `results/runtime-gate/phase290-groq-compound-mini-paced-inconsistent-pair-control/live/`. Focused tests, race test for the CLI package, full `go test ./...`, full `go vet ./...`, manifest/report JSON decoding, timing inspection, `git diff --check`, and secret-safe artifact inspection passed.

## Phase 291 — paced Llama 3.3 inconsistent-pair cross-model control (2026-07-27 21:40 -03)

**Hypothesis and bounds.** Rotate the paced inconsistent-evidence campaign from Groq Compound Mini to `llama-3.3-70b-versatile` while preserving the Phase 290 prompt, five-field structural oracle, three isolated SQLite stores, exactly one call per trial, zero retry/fallback, 128 output-token ceiling, 45 s call timeout, seeded NIM circuit, and a declared 30 s delay between trials. This tests whether safe `reconcile_safe=false` semantics and provider availability survive pacing on a different architecture; it does not estimate an unpaced-vs-paced causal effect for Llama 3.3.

**Observed evidence.** All 3/3 calls completed with `finish_reason=stop`, 221 input + 43 output tokens each and provider latency 336.5–362.9 ms (p50 341.0 ms; p95/max 362.9 ms). Every response was integral valid JSON and matched all 5/5 protected fields structurally: `risk=unknown_effect`, both evidence names as a set, `complete_pair=true`, `reconcile_safe=false`, and `canonical_writes=0`. Exact bytes differed only in serialization. Local quota blocked every second acquisition, all operations parked authority-free in `WAITING_TIME`, no canonical entity/commit was promoted, and all stores reopened durably. Two earlier invocations failed locally (unsupported shell option/stale dotenv path, then an incorrect CLI flag) before the runner contacted any provider; they are not live observations.

**Interpretation and decision.** The paced cross-model control is 3/3 available and semantically safe, versus Compound Mini's paced 3/3 in Phase 290; Llama 3.3 used 792 observed tokens total versus Compound Mini's substantially higher agentic overhead and had sub-400 ms provider latency in this sample. This descriptive comparison does not authorize routing preference, automatic retry, or a general safe request rate. Preserve strict parsing, zero retry, kernel-owned disposition, and manifest-declared pacing for deliberate batches. The next useful work is crash-resumable pacing with a persisted `next_trial_not_before` and duplicate-call reconciliation, proven offline before another live stress batch; do not repeat this now-stable case without a changed hypothesis.

**Evidence and verification.** `results/runtime-gate/phase291-groq-llama33-paced-inconsistent-pair-control/live/` contains the manifest, aggregate, three sanitized receipts, and three durable SQLite stores. Independent receipt inspection confirmed three calls/successes/reopens, 15/15 protected-field matches, 663 input + 129 output tokens, quota enforcement, and zero promotion.

## Phase 292 — persisted pacing checkpoint and bounded Groq smoke (2026-07-27 22:00 -03)

**Implemented.** The runtime-gate batch CLI now persists a versioned `pacing-state.json` after each durable trial. It records campaign identity, planned/completed trial counts and `next_trial_not_before`; restart validates identity, reloads only reports already marked `durable_reopen`, resumes at the first incomplete trial and waits against the persisted deadline. This prevents a process restart from silently duplicating completed provider calls. State publication uses same-directory temporary write plus atomic rename; mismatched or incomplete state fails closed.

**Live bounded evidence.** Rotated from Groq Llama 3.3 70B to `llama-3.1-8b-instant` for one inconsistent-pair smoke call, zero retry/fallback, 128 output-token ceiling, 45 s timeout, NIM seeded circuit-open and a declared 5 s pacing interval. The provider returned successfully with `stop`; exact bytes varied, while the redaction-safe structural oracle matched all 5/5 protected fields, including `reconcile_safe=false` and `canonical_writes=0`. The operation remained authority-free and SQLite reopened durably. This n=1 run validates the current path, not provider-rate causality or routing preference.
...
## Phase 295 — live crash-after-report adoption avoids provider replay (2026-07-27 23:00 -03)

**Hypothesis.** Crashing the subprocess after the first trial has published its durable report but before the pacing checkpoint marks it complete must be recoverable by adopting that one-call report. Restart must neither replay trial 1 nor begin trial 2 before the deadline reconstructed from the adopted report.

**Live bounded campaign.** Groq `llama-3.1-8b-instant`; two isolated inconsistent authenticated-pair trials; one call per trial; 5 s persisted inter-trial delay; 45 s timeout; 128 output-token ceiling; zero retries/fallback; NIM circuit-open; strict integral JSON; zero canonical writes. The first subprocess exited through the opt-in post-report fault point. Restart adopted trial 1 from its durable one-call report, reconstructed pacing, then issued trial 2. Both provider calls succeeded, both completions matched all 5/5 protected fields structurally, both stores reopened durably, and the aggregate records exactly two external calls for two trials. No canonical state was promoted.

**Decision.** The report-publication side of the ambiguous boundary now has end-to-end live crash/restart evidence: a complete durable report is sufficient to reconcile the in-flight intent without duplicate provider work. Preserve fail-closed behavior when that report is absent or incomplete; provider-supported idempotency remains necessary before any automatic replay across the truly ambiguous post-dispatch/pre-report window.

**Evidence and verification.** `results/runtime-gate/phase295-groq-llama31-report-crash-adoption/live/`. Independent JSON inspection confirmed two completed trials, two calls, two provider successes, two durable reopens, cleared in-flight state, 442 input + 108 output tokens, and the final pacing deadline. Focused/full tests, vet, JSON validation, secret scan, and `git diff --check` are executed before commit.

## Phase 296 — Llama 3.1 complete-pair type failure is stable across a bounded replay (2026-07-27 23:23 -03)

**Hypothesis and bounds.** Measure whether the Phase 280 Groq `llama-3.1-8b-instant` complete authenticated-pair type mismatch was incidental. Replayed the identical prompt and four-field structural oracle in five isolated SQLite trials, one external call per trial, 45 s timeout, 64 output-token ceiling, zero retries/fallback, seeded primary circuit, and early-stop after two repeated provider failures. The campaign permits no canonical writes and does not delegate recovery disposition.

**Observed evidence.** All 5/5 provider calls completed with `stop`; latency was 350.2 ms p50, 469.2 ms p95/max, and usage totaled 945 input + 240 output tokens. Every 163-byte completion was integral valid JSON and matched `risk`, `complete_pair`, and `canonical_writes`, but all 5/5 represented `available_evidence` with the wrong JSON type. The sanitized structural oracle therefore recorded the same `type_mismatch` in every trial (15/20 protected fields matched overall). Exact equality was 0/5, local quota blocked every second acquisition, all five stores reopened durably, and no canonical state was promoted.

**Interpretation and decision.** The complete-pair collection-type failure is stable in this bounded sample rather than a one-off serialization variation, while the model still preserves the three scalar safety fields. This strengthens the evidence-only classification for this deployment on the executable complete-pair contract and confirms that field-level correctness cannot substitute for array typing. Preserve strict parsing, no scalar/object coercion, zero retry, and kernel-owned disposition. Do not repeat this now-characterized prompt on Llama 3.1 8B; the next useful experiment is a changed schema/prompt hypothesis or a cross-provider control after deliberate rotation.

**Evidence and verification.** `results/runtime-gate/phase296-groq-llama31-complete-pair-variance/live/` records five calls/successes, five identical redaction-safe type mismatches, five durable reopens, quota enforcement, and zero promotion. JSON decoding, focused/full tests, vet, diff checking, and secret-safe artifact inspection are executed before commit.

## Phase 297 — explicit array contract repairs the stable Llama 3.1 type failure (2026-07-27 23:40 -03)

**Hypothesis and bounds.** The stable 5/5 `available_evidence` type mismatch from Phase 296 may be repairable at the prompt boundary without weakening the executable parser. The changed prompt now states the JSON types explicitly and requires `available_evidence` to be a JSON array of exactly two JSON strings, never an object, map, or scalar. Groq `llama-3.1-8b-instant` ran three isolated trials, one external call each, 45 s timeout, 64 output-token ceiling, zero retries/fallback, seeded primary circuit, strict integral JSON, structural set comparison, zero canonical writes, and durable reopen.

**Observed evidence.** All 3/3 provider calls completed with `finish_reason=stop`; latency was 294.8 ms p50 and 300.1 ms p95/max, with 642 input + 123 output tokens total. Every 143-byte completion was integral valid JSON, had an identical sanitized SHA-256, and matched all 4/4 protected fields structurally, including `available_evidence` as the required array. Exact bytes were 0/3 because serialization differed from the canonical oracle, while authority-free execution succeeded, local quota blocked each second acquisition, and all three SQLite stores reopened durably. No canonical state was promoted.

**Interpretation and decision.** Relative to Phase 296's unchanged 5/5 type mismatch, the explicit JSON-type instruction converted the measured outcome to 3/3 structurally adherent responses without parser coercion, retry, or expanded model authority. This is strong local evidence that the failure was prompt-contract sensitivity rather than inability to emit arrays, but the small changed-prompt sample does not authorize general routing preference. Adopt the explicit type wording for this operation's prompt contract, preserve strict integral parsing and array typing, and keep recovery disposition kernel-owned. The next useful experiment is a rotated-model control with the changed prompt or an adversarial three-item/duplicate evidence set; do not weaken the parser based on serialization-only byte mismatch.

**Evidence and verification.** `results/runtime-gate/phase297-groq-llama31-explicit-array-repair/live/` records three calls/successes, twelve of twelve protected-field matches, three durable reopens, quota enforcement, and zero promotion. Manifest/report JSON decoding, manifest identity, sanitized receipt inspection, focused/full Go tests, vet, secret scan, and `git diff --check` are executed before commit.
