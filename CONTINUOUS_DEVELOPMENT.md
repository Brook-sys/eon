...
- [x] `DONE` Implementar command inbox idempotente com pause/resume/cancel/shutdown e gate do scheduler.
  ...
  - Residual: crash-replay subprocessado do processador de comandos e API HTTP de submit de comandos ainda abertos; inspeção HTTP somente-leitura já coberta por `internal/inspect`.
- [x] `DONE` Completar external event inbox genérico (mensagem/despertar) com persistência e processador kernel.
  ...
  - Residual: superfícies HTTP de submit de eventos/comandos e crash-replay subprocessado dos processadores ainda abertos.
