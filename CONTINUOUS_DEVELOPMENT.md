...
- [ ] Vincular identificador de sessão/request gerado localmente na query string (ex: `request_id=`) no handhshake do `EventSource` no cliente para evitar vazamento de estado cruzado.
- [ ] Validar e injetar o `request_id` filtrado no pipeline SSE (`Projector`), complementando o isolamento iniciado por `namespace`.
- [ ] Provar ausência de contaminação cruzada enviando writes não correlatos no mesmo namespace mas em requisições paralelas mockadas.

2026-07-22 12:40 - HEARTBEAT - Planejando a Fase 100. O isolamento lógico iniciado pela introdução de `namespace` cobriu a distinção macro, mas dentro de um mesmo escopo o cliente pode reconectar e acidentalmente consumir streams em voo de conexões antigas ainda ativas. O foco será adicionar um token rastreável `request_id` gerado pelo cliente e persistido internamente pelo `Projector` associando callbacks efêmeros do SSE a instâncias seguras. Campanha de teste live usando `cliproxyapi/topmodel` em escopo `cognitive-v2` obteve PROVIDER_ERROR maciço (66 runs reprovados, report movido para `continuous-probe-2026-07-22-1240-topmodel-sse-live-fail`), reafirmando a regra firme que resultados offline não alteram baseline em vivo e demonstrando liveness do runner no novo pipeline. Avançaremos com a implementação e testes unitários.
