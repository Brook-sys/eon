...
- [ ] `TODO` Criar um harness integrado ao `ModelExecutor` (similar ao `malformed-recovery`) que injete respostas malformadas para as etapas inicial e de short correction, avançando a execução até o estágio `SIMPLER_FORMAT` e só então utilizando um provider live.
- [ ] `TODO` Executar uma campanha live bounded (teto de 1 chamada externa) no Groq ou NVIDIA NIM focada em provar a aderência do modelo de fallback ao formato JSON simplificado quando esquemas complexos falham.
