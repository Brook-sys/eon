...
- [ ] `IN_PROGRESS` Implementar gerador, runner medido e crash harness subprocessado para ambos os backends.
  ...
  - Evidência parcial adicional: worker aceita fixture oficial versionada e executa toda a linhagem + changeset + seis registros observáveis em uma única transação; crash SQLite antes/depois do commit é classificado por reopen como `NOT_APPLIED`/`APPLIED`, nunca parcial.
  - Restante: exercer a fixture oficial em Dolt e implementar Dolt medido via `sql-server`, distinguindo working set SQL de commit Dolt.
