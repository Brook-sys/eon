...
## Observação live (2026-07-22)

Neste heartbeat implementamos e verificamos a concorrência e integridade atômica da gravação concorrente de logs no banco. Para atestar a preservação live executamos uma campanha live via `cmd/model-benchmark-runner -mode=live -model=llama-3.1-8b-instant -base-url=https://api.groq.com/openai/v1` com Groq propositalmente sem token setado corretamente; capturamos exatos 66 retornos HTTP 401 sem falhas na validação local ou mutações impuras do event log. As rotinas estáticas estão provadas limpas com pass incondicional de toda suite Go test local e runtime mantendo propriedades ACID.
