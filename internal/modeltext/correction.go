...
const DelimitedChangeSetFormat = `CHANGESET_DELIMITED_V1 followed by exactly one line per property matching KEY: JSON_VALUE for schema_version, id, mission_revision_id, operation_id, base_commit_id, read_set, preconditions, changes, expected_delta, validator_ids, provenance, idempotency_key; keys must be strictly lowercase exactly as spelled here; every value after ':' must be valid JSON on that same line; schema_version must be decimal integer 1; id, mission_revision_id, operation_id, base_commit_id, expected_delta, provenance, and idempotency_key must be quoted JSON strings; read_set, preconditions, and validator_ids must be JSON arrays of strings; changes must be a JSON array of objects containing only quoted string fields kind, entity_type, entity_id, payload_ref; use compact arrays/objects; include all 12 lines exactly once even when an array is empty; do not include unknown fields, file contents, or other properties; do not output Markdown fences or prose`
