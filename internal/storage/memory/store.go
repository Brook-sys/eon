...
func (t transaction) SubagentStatusDeliveriesRequiringReconciliation(now time.Time, limit int) ([]domain.SubagentSpawnReceipt, error) {
	return reader(t).SubagentStatusDeliveriesRequiringReconciliation(now, limit)
}
func (r reader) SubagentStatusDeliveriesRequiringReconciliation(now time.Time, limit int) ([]domain.SubagentSpawnReceipt, error) {
	if now.IsZero() || limit <= 0 {
		return nil, fmt.Errorf("effect-unknown subagent status query requires positive limit")
	}
	result := make([]domain.SubagentSpawnReceipt, 0, limit)
	for _, receipt := range r.state.subagentSpawnReceipts {
		if (receipt.StatusDelivery == domain.SubagentStatusDeliveryEffectUnknown || receipt.StatusDelivery == domain.SubagentStatusDeliveryInFlight) && (receipt.ReconcileAfter.IsZero() || !now.Before(receipt.ReconcileAfter)) {
			result = append(result, receipt)
		}
	}
	sort.Slice(result, func(i, j int) bool {
...
func (t transaction) EffectUnknownSubagentDispatches(now time.Time, limit int) ([]domain.SubagentDispatch, error) {
	return reader(t).EffectUnknownSubagentDispatches(now, limit)
}
func (r reader) EffectUnknownSubagentDispatches(now time.Time, limit int) ([]domain.SubagentDispatch, error) {
	if now.IsZero() || limit <= 0 {
		return nil, fmt.Errorf("effect-unknown subagent dispatch query requires positive limit")
	}
	result := make([]domain.SubagentDispatch, 0, limit)
	for _, dispatch := range r.state.subagentDispatches {
		if dispatch.Status == domain.SubagentDispatchEffectUnknown && (dispatch.ReconcileAfter.IsZero() || !now.Before(dispatch.ReconcileAfter)) {
			result = append(result, dispatch)
		}
	}
	sort.Slice(result, func(i, j int) bool {
