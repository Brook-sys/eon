...
func TestApplyCommitCascadesStaleOnParentArtifactReplacementSurvivesRestart(t *testing.T) {
	t.Parallel()
	...
	if err := store2.View(ctx, func(r port.Reader) error {
		...
		if !derived.Stale {
			t.Fatal("derived plan was not marked stale after parent artifact was replaced across restart")
		}
		return nil
	}); err != nil {
		t.Fatal(err)
