...
func seedDispatch(t *testing.T, store port.Store, now time.Time) domain.SubagentDispatch {
	t.Helper()
	record := domain.SubagentRecord{SchemaVersion: domain.SchemaVersionV1, ID: "session-1", TaskID: "task-1", MissionID: "mission-1", State: domain.SubagentStatePending, StartedAt: now, UpdatedAt: now, Task: "remote task", ContextMode: "isolated", TransportPeerID: "peer-a", MaxAttempts: 2, Deadline: now.Add(time.Minute)}
	dispatch := domain.SubagentDispatch{SchemaVersion: domain.SchemaVersionV1, RequestID: "dispatch-1", SessionID: record.ID, PeerID: "peer-a", Status: domain.SubagentDispatchPending, MaxSendAttempts: 3, AvailableAt: now, CreatedAt: now, UpdatedAt: now}
