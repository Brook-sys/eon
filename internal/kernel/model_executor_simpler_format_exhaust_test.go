...
func TestModelExecutorAlwaysInvalidExhaustsWithSimplerFormat(t *testing.T) {
	t.Parallel()
	...
	err := store.Update(ctx, func(tx port.Transaction) error {
		...
		spec := modelTestSpec()
		spec.Budget.ModelCalls = 3 // 1: normal, 2: short, 3: simpler -> exhaust
	...
	server := fakeserver.New(
		...
		fakeserver.Exchange{ResponseText: "simpler format is not json", ResponseModel: "fixture-model"},
		fakeserver.Exchange{ResponseText: "should not be called", ResponseModel: "fixture-model"},
	)
	defer server.Close()
	...
	if !hasShort || !hasSimpler {
		t.Fatalf("expected short and simpler stages, got %v", result.RecoveryStages)
	}
}
