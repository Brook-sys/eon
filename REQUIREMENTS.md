...
**Estado (2026-07-16 / 2026-07-17):** implementado para `model.complete` (PROPOSE_ONLY), path web `web.search`/`web.fetch` e path file `file.discover`/`file.read` (READ_ONLY). Domínio: `Budget.Covers`/`Consume`/`Remaining` (zero = nenhuma unidade autorizada, nunca ilimitado); `CapabilityCatalog` + `EvaluateCapability` (ALLOW/DENY/REQUIRE_APPROVAL, TTL, digest de args, permissões, budget); `Acquire`/`ReportSuccess`/`ReportFailure` ResourceGate com cota/min/dia, tokens/min, concorrência + slots reservados, circuit breaker e Retry-After; `ThrottleTransitionInput` → `WAIT_UNTIL`/`THROTTLE`; `NewResourceBudgetFailure`; `SchedulerCadenceConfig`/`DefaultSchedulerCadenceConfig`/`WithinCycleBudget` (MaxDispatches + MaxCycleDuration + idle bounds). Kernel: `CapabilityAuthorizer.ReserveCapability`/`ReportCapability` genéricos (`ReserveModelComplete`/`ReportModelComplete` wrappers; custos web/file `WebSearchCost`/`WebFetchCost`/`FileDiscoverCost`/`FileReadCost`); `ModelExecutor`, `WebExecutor` e `FileExecutor` reservam antes do lease, aplicam throttle sem chamar adapter e liberam o slot em todos os exits; `FileExecutor` confina paths a roots absolutas autorizadas (sem `..`, sem escape por symlink); `DispatchExecutor` roteia Local → File → Web → Model (`requires_file`/`requires_web` quando elegível sem adapter); `ActiveSchedulerCadence`/`ResolveSchedulerCadence` projetam revisões `SCHEDULER`. Bootstrap: `buildModel`/`buildWeb`/`buildFile` + flags `-web*`/`-file*`/`-file-roots` no `cmd/runtime`; `ProcessCycle` multi-step sob `MaxDispatches`/`MaxCycleDuration` com `DispatchBudgetHit`/`CycleBudgetHit`; `RunControlLoop` aplica idle min/max da cadence ativa. Persistência: portas `ResourceUsage(s)`/`SaveResourceUsage` no store memory (+ checkpoint binário SQLite). Inspect read-only: `GET /resources` e `GET /resources/{resourceID}`. Residual FR-RES-001: live model eval permanece READY sem endpoint local.
...
**Verificação de checkpoint (2026-07-18):** o checkpoint integral v2 possui envelope gob com `CheckpointFormatVersion`, payload interno separado e SHA-256 verificado antes da restauração; SQLite e ambos os adapters Dolt persistem e verificam versão externa compatível antes de decodificar. Versões futuras são rejeitadas fail-closed por erro identificável, adulteração do payload produz erro de integridade, payload corrente corrompido não é tratado como store vazio, e formatos v0/v1 permanecem legíveis para migração automática para v2 na próxima escrita bem-sucedida. O backup SQLite só é aceito após `PRAGMA integrity_check(1)`, `PRAGMA foreign_key_check` vazio, inventário físico coerente (`page_size * page_count == file_size`), concordância da versão externa, inventário de exatamente um objeto de schema de aplicação (`runtime_checkpoint`), total de no máximo uma linha com o ID canônico `1`, verificação dos digests físico/schema/lógico e decode integral do checkpoint; a mesma auditoria pode ser repetida sem migração por `sqlite.VerifyBackup`. Verificação e restore aceitam fixar separadamente essas identidades, inclusive page size/count, e rejeitam constraints violadas, IDs não canônicos, tabelas, índices, views ou triggers extras, mesmo quando a tabela canônica ainda é decodificável.
