...
As campanhas oficiais completas foram executadas em 2026-07-15 com Dolt 2.2.0. Os artefatos auditáveis em `results/dolt-server/2026-07-15/crash` preservam todos os trials e agregados: antes de `SQL COMMIT`, 30/30 foram `NOT_APPLIED`; após `SQL COMMIT` e antes de `DOLT_COMMIT`, 30/30 foram `INVALID_PARTIAL`; após `DOLT_COMMIT`, 30/30 foram `APPLIED`. Portanto, a separação entre commit SQL e commit Dolt não é apenas risco teórico: na configuração atual ela viola o bloqueador absoluto de ausência de estado parcial. Dolt permanece candidato somente se o adapter eliminar, reconciliar deterministicamente ou tornar inobservável essa janela sem enfraquecer o contrato. O adapter Dolt por CLI não será usado como número principal porque inclui startup por atualização e une SQL write a `DOLT_COMMIT` numa única invocação. Ainda falta executar o workload medido comum e integrar latência, footprint, diff e complexidade ao relatório.
...
Resultado decisório provisório: Dolt falhou um bloqueador absoluto por `INVALID_PARTIAL`; desempenho bruto, diff ou branch não podem compensar essa falha. SQLite passa o contrato e as campanhas de crash atuais e é o candidato operacional seguro. A ADR final ainda exige decidir explicitamente se haverá uma tentativa delimitada de protocolo de reconciliação Dolt; sem isso, deve registrar SQLite como backend do MVP e Dolt como alternativa rejeitada nesta configuração. Métricas backend-native de diff/branch/merge permanecem secundárias porque o bloqueador já impede seleção de Dolt e não devem prolongar indefinidamente o spike.
