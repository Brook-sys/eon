...
Os adapters MUST rejeitar versões futuras ou desconhecidas sem tentar interpretá-las. Payload inválido na versão corrente MUST falhar a abertura, nunca inicializar silenciosamente um store vazio. A versão corrente v2 separa o estado serializado do envelope e registra SHA-256 sobre os bytes internos; qualquer alteração do payload sem atualização coerente do digest falha como erro de integridade antes da restauração. O decoder também exige framing exato de um único documento gob e rejeita bytes ou documentos concatenados. A versão da coluna e a identidade interna MUST concordar: coluna v2 aceita somente envelope v2; coluna v1 aceita somente o gob legado sem envelope (v0) ou envelope v1. O checkpoint gob legado sem envelope e o envelope v1 permanecem legíveis e são substituídos pelo envelope v2 na próxima atualização durável; adapters escrevem somente v2. Mudanças incompatíveis futuras exigem função de migração explícita e testes com fixtures da versão anterior; alterar apenas a constante não constitui migração.
