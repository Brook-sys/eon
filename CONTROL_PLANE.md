...
- consulta paginada ao event log — implementado (`GET /events` com `after_sequence`, `limit` e filtros de correlação; `GET /events/{id}`); filtros esparsos são varridos em lotes bounded até o próximo match, EOF ou teto global de 5.000 eventos, e `next_sequence` avança pelo trecho efetivamente examinado para não repetir gaps nem saltar evidência correlacionada;
- detalhe correlacionado de operação e commit — implementado (`GET /operations/{id}`, `GET /commits/{id}`, `GET /commands/{id}` via projeções somente-leitura sobre store + event log; raw model outputs correlacionados por `ValidationReceipt.ArtifactRef`; resumo FR-MODEL-004 em `model_recovery` e FR-MODEL-006/007 em `model_adaptation` derivados de eventos `operation.model_*` — recovery decisions/invocations/fallback/exhausted e planos level/format/ctx — sem mutar o store); eventos de efeito de comandos carregam `payload_ref=<command_id>:<result_ref>`, pois `result_ref` de domínio pode se repetir entre comandos idempotentes e não é identidade suficiente; as três respostas expõem `events_truncated` quando os tetos bounded impedem provar completude, e o dashboard mostra aviso no resumo e no painel de eventos em vez de apresentar o prefixo como auditoria completa; cargas concorrentes do inspector são generation-fenced no cliente, impedindo resposta ou erro stale de substituir a seleção mais recente;
...
- SSE ao vivo — implementado (`GET /events/stream` com `after_sequence`/`Last-Event-ID`, poll configurável, eventos `ready`/`event`/`page`/`terminal_error`/`cursor_ahead` e keep-alive; somente-leitura sobre o store e `Projector.ListEvents`); cada conexão exige primeiro uma decisão de baseline: `ready` certifica o cursor solicitado, enquanto `cursor_ahead` pode rejeitá-lo somente antes dessa certificação; `event`, `page` ou `terminal_error` antes do primeiro `ready`, assim como `cursor_ahead` posterior, fecham fail-closed sem alterar cursor nem apresentar evidência; antes de certificar `ready`, o servidor compara o cursor solicitado ao high-water durável e, se estiver à frente, emite uma única resposta versionada `cursor_ahead` com cursor solicitado e high-water em decimal exato, encerra o stream e exige reset explícito — nunca mantém uma conexão aparentemente viva e cega, nem faz clamp silencioso após restore/substituição do store; todos os frames nomeados declaram `schema_version: 1`, e o dashboard fecha fail-closed antes de alterar cursor ou apresentar evidência quando um payload JSON parseável traz versão ausente/incompatível ou espelho decimal divergente; erro nativo de transporte antes do primeiro `ready` também fecha e cerca a conexão, em vez de reconectar sem baseline certificada; depois de `ready`, o dashboard nunca deixa o mesmo `EventSource` escolher a retomada automaticamente: fecha e cerca a fonte nativa, captura naquele instante o último cursor aceito pela aplicação, agenda retry bounded com backoff de 250 ms a 5 s e cria uma fonte nova usando exclusivamente essa captura imutável — edições posteriores do campo manual `after_sequence` não podem alterar, cancelar nem redirecionar a recuperação automática; o orçamento de recovery só é resetado por avanço útil de `event`/`page`, nunca por uma fronteira `page` igual ao cursor corrente, evitando loops ilimitados `ready(N) → page(N) → error`; o primeiro `ready` dessa fonte de retry deve ecoar exatamente essa baseline — avanço ou recuo revela retomada divergente e fecha fail-closed; tentativa manual é transacional e só cancela timer/reset de orçamento depois que cursor e construção da fonte candidata passam, enquanto falha síncrona ao construir uma fonte de retry termina visivelmente e exige reconnect manual; payload `event` que nem sequer é JSON ainda avança pelo ID já aceito pelo `EventSource` e recebe rótulo explícito de malformado, evitando replay sem confiar em campos da aplicação; o frame `ready` conserva no próprio `id` o cursor de retomada aceito (com `Last-Event-ID` prevalecendo), para uma desconexão anterior ao próximo evento não redefinir o cursor do navegador; em streams filtrados, o cursor interno avança por `page.next_sequence` mesmo quando a página não emite matches, evitando revarrer indefinidamente uma janela esparsa bounded antes de evidência posterior; backlog é drenado imediatamente em bursts de no máximo 8 páginas, com yield pelo poll timer entre bursts quando ingestão contínua mantém `has_more=true`; todos os frames que carregam cursor espelham o `id` em decimal textual exato (`after_sequence_decimal`, `sequence_decimal`, `next_sequence_decimal` ou `high_water_decimal`) para preservar toda a faixa `uint64` em clientes JavaScript, e o dashboard valida a igualdade antes de aceitar fronteiras ou renderizar evidência; frames `event` exigem avanço estrito do ID — igualdade continua admitida apenas em fronteiras `ready`/`page`, e evento repetido fecha sem renderizar o payload como evidência nova; `terminal_error` encerra falha de aplicação explicitamente, carrega o último cursor aceito no `id` e em `after_sequence_decimal`, e só é apresentado pelo dashboard quando ambos coincidem exatamente com a posição local; divergência fecha como erro de protocolo sem confiar no payload, enquanto o evento nativo reservado `error` permanece reconnectável pelo dashboard somente após baseline certificada;
...
Browse de conhecimento no dashboard experimental: seção **Conhecimento** com catálogo (`GET /api/inspect/knowledge`), listas e detalhe de claims/sources/observations/artifacts via Control API; filtros avançados (q/kind/provenance, claims sem evidência/com contradição, observations linkadas, artifacts stale) e marcador de contradição na lista; sem escrita canônica e sem snapshot bytes. Redaction de free-text de knowledge também é aplicada na API.
