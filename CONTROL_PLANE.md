...
- detalhe correlacionado de operação e commit — implementado (`GET /operations/{id}`, `GET /commits/{id}`, `GET /commands/{id}` via projeções somente-leitura sobre store + event log; raw model outputs correlacionados por `ValidationReceipt.ArtifactRef`; resumo FR-MODEL-004 em `model_recovery` derivado de eventos `operation.model_*` — decisions, invocations, fallback/recovery tags, exhausted — sem mutar o store);
