...
- detalhe correlacionado de operação e commit — implementado (`GET /operations/{id}`, `GET /commits/{id}`, `GET /commands/{id}` via projeções somente-leitura sobre store + event log; raw model outputs correlacionados por `ValidationReceipt.ArtifactRef`; resumo FR-MODEL-004 em `model_recovery` e FR-MODEL-006/007 em `model_adaptation` derivados de eventos `operation.model_*` — recovery decisions/invocations/fallback/exhausted e planos level/format/ctx — sem mutar o store); as três respostas expõem `events_truncated` quando os tetos bounded impedem provar completude, e o dashboard mostra aviso no resumo e no painel de eventos em vez de apresentar o prefixo como auditoria completa;
