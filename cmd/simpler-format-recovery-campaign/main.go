...
func run() error {
	manifestPath := flag.String("manifest", "", "simpler format recovery campaign manifest")
	...
	var manifest gatecampaign.SimplerFormatRecoveryCampaignManifest
	if err := decoder.Decode(&manifest); err != nil {
		return fmt.Errorf("decode simpler format recovery manifest: %w", err)
	}
	var extra any
	if err := decoder.Decode(&extra); err == nil {
		return errors.New("simpler format recovery manifest contains trailing data")
	} else if err != io.EOF {
		return fmt.Errorf("decode trailing simpler format recovery manifest data: %w", err)
	}
	...
	if err := gatecampaign.WriteSimplerFormatRecoveryCampaignManifest(filepath.Join(*outputDirectory, "manifest.json"), manifest); err != nil {
		return err
	}
	...
	defer cancel()
	report, runErr := (gatecampaign.SimplerFormatRecoveryCampaignRunner{Store: store, Clock: clock, Providers: providers}).Run(ctx, manifest)
	...
	if err != nil {
		return fmt.Errorf("reopen simpler format recovery store: %w", err)
	}
	if err := gatecampaign.VerifySimplerFormatRecoveryDurability(context.Background(), reopened, report); err != nil {
		reopened.Close()
		return err
	}
	...
	if err := gatecampaign.WriteSimplerFormatRecoveryCampaignArtifacts(*outputDirectory, report); err != nil {
		return err
	}
