...
func main() {
	...
	outDir := filepath.Join("results", "phase398-auto-scale-parser")
	...
	cases := []struct {
		...
		{
			name:         "bulleted_markdown_key_parsing",
			instruction:  "Extract system status. Return formatted lines:\n- BUILD: SUCCESS\n* ERRORS: 0_ERRORS\n1. TARGET: PROD",
		...
		{
			name:         "reasoning_budget_auto_scale_test",
	...
	for _, m := range models {
		...
		for _, c := range cases {
			...
			if err != nil {
				...
				log.Printf("[%s/%s] Task %s FAILED in %dms: %v", m.provider, m.model, c.name, elapsed, err)
			} else {
				...
				log.Printf("[%s/%s] Task %s OK in %dms | strategy=%s compliance=%.2f semantic=%v",
					m.provider, m.model, c.name, elapsed, parseRes.Strategy, parseRes.FormatComplianceScore, trial.SemanticCorrect)
			}
	...
	reportMarkdown := fmt.Sprintf(`# Phase 398 — Auto-Scale & Bullet Parser Fire Campaign Report
...
`, time.Now().UTC().Format(time.RFC3339), summary.TotalTrials, summary.SuccessCount, summary.SuccessRate*100, summary.P50LatencyMS, summary.P95LatencyMS, summary.ComplianceAvg)
	...
	for _, t := range trials {
		reportMarkdown += fmt.Sprintf("| %s | %s | %s | %s | %s | %.2f | %v | %dms |\n",
