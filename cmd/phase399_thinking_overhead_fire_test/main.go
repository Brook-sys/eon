...
func main() {
	...
	outDir := filepath.Join("results", "phase399-thinking-overhead-parser")
	...
	for _, m := range models {
		...
		for _, c := range cases {
			...
			if err != nil {
				...
				log.Printf("[%s/%s] Task %s OK in %dms | strategy=%s compliance=%.2f semantic=%v suppressed=%v",
					m.provider, m.model, c.name, elapsed, parseRes.Strategy, parseRes.FormatComplianceScore, trial.SemanticCorrect, compResult.ReasoningEffortSuppressed)
			}
	...
	reportMarkdown := fmt.Sprintf(`# Phase 399 — Thinking Overhead & Bold Parser Fire Campaign Report
...
`, time.Now().UTC().Format(time.RFC3339), summary.TotalTrials, summary.SuccessCount, summary.SuccessRate*100, summary.P50LatencyMS, summary.P95LatencyMS, summary.ComplianceAvg)
	...
	for _, t := range trials {
		reportMarkdown += fmt.Sprintf("| %s | %s | %s | %d | %v | %s | %.2f | %v | %dms |\n",
			t.Provider, t.Model, t.TaskCase, t.ThinkingOverhead, t.ReasoningSuppressed, t.ParseStrategy, t.ComplianceScore, t.SemanticCorrect, t.LatencyMS)
	}

	_ = os.WriteFile(filepath.Join(outDir, "REPORT.md"), []byte(reportMarkdown), 0644)
	fmt.Printf("\n=== Phase 399 Campaign Complete ===\nReport saved to %s/REPORT.md\n", outDir)
}
