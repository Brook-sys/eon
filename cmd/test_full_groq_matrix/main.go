...
func main() {
	...
	models := []struct {
		...
		tpm       int
		reasoning string
		antiForge bool // whether to enable anti-forgery for this model
	}{
		{"llama-3.3-70b-versatile", 30, 30000, "", true},   // needs anti-forgery
		{"qwen/qwen3.6-27b", 40, 40000, "none", false},     // works without
		{"llama-3.1-8b-instant", 30, 30000, "", false},     // test if 8B needs it
	}
	...
	for _, m := range models {
		pInput := prompt.Input{
			...
			FormatAntiForgeryGuard: m.antiForge,
		}
		...
		fmt.Printf("\n=== CONCURRENT TEST: %s (RPM=%d, Semaphore=3, c=%d, anti-forgery=%s) ===\n", m.name, m.rpm, concurrency, antiStr)
		...
		for i := 1; i <= concurrency; i++ {
			...
			go func(idx int) {
				...
				start := time.Now()
				resp, err := client.Complete(ctx, compiledReq)
