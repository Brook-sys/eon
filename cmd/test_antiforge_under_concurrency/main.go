...
func main() {
	...
	// Test configurations: without and with FormatAntiForgeryGuard
	testConfigs := []struct {
		name         string
		antiForgery  bool
	}{
		{"WITHOUT anti-forgery", false},
		{"WITH anti-forgery (V1)", true},
	}
	...
	models := []struct {
		...
		// {"qwen/qwen3.6-27b", 40, 40000, "none"}, // Qwen already works, focus on 70B
	}
	...
	for _, tc := range testConfigs {
		for _, m := range models {
			pInput := prompt.Input{
				...
				FormatAntiForgeryGuard: tc.antiForgery,
			}
			...
			fmt.Printf("\n=== CONCURRENT TEST: %s | %s (RPM=%d, Semaphore=3, c=%d) ===\n", m.name, tc.name, m.rpm, concurrency)
