...
func main() {
	...
	scenarios := []struct {
	...
	}{
		{
			...
			Prompt: `The executor initiated a state transition from INITIALIZING to READY. No errors were encountered. 
...
REASON: [Brief explanation]`,
		...
		},
		{
			...
			Prompt: `O executor tentou uma transição de estado de RUNNING para STOPPED, mas a operação de gravação de log falhou por falta de espaço em disco e ele abortou a transição.
...
REASON: [Explicação breve]`,
	...
	for _, model := range models {
		for _, scenario := range scenarios {
			for i := 0; i < 3; i++ { // 3 trials per model/scenario
				wg.Add(1)
				go func(m, s string, p string, exp string, maxTokens int, endpoint, key string) {
					defer wg.Done()
					start := time.Now()
					...
					mu.Unlock()
				}(model.ID, model.Provider, scenario.Prompt, scenario.ExpectedState, scenario.MaxTokens, model.Endpoint, model.Key)
			}
		}
	}
	...
	outDir := filepath.Join("/home/node/.openclaw/workspace/motor-autonomo/results", "phase438-relaxed_parsing_status_test")
