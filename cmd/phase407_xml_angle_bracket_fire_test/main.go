...
func main() {
	log.Println("=== Phase 407: XML/HTML Angle Bracket Key Parsing Live Fire Campaign ===")
	...
	outDir := filepath.Join("results", "phase407-xml-angle-bracket-parser")
	...
	testCases := []TestCase{
		{
			name:         "xml_angle_bracket_keys",
			expectedKeys: []string{"DATE", "SOURCE", "STATUS"},
			instruction:  "Extract information using XML tag key names. Format as <DATE>: 2026-08-08, <SOURCE>: Audit Report Alpha, <STATUS>: VERIFIED.",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(v["DATE"], "2026-08-08") &&
					strings.Contains(strings.ToLower(v["SOURCE"]), "audit report") &&
					strings.Contains(strings.ToUpper(v["STATUS"]), "VERIFIED")
			},
		},
		{
			name:         "bulleted_xml_keys",
			expectedKeys: []string{"REASON", "RECOMMENDATION"},
			instruction:  "Format your response as bullet points with XML tag key names: - <REASON>: High latency in database query, - <RECOMMENDATION>: Add composite index.",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(strings.ToLower(v["REASON"]), "latency") &&
					strings.Contains(strings.ToLower(v["RECOMMENDATION"]), "index")
			},
		},
		{
			name:         "bold_xml_mixed_keys",
			expectedKeys: []string{"METRIC", "VALUE", "VERDICT"},
			instruction:  "Extract metrics with mixed bold XML tags: **<METRIC>**: Memory Footprint, **<VALUE>**: 3.90x, **<VERDICT>**: REJECTED.",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(strings.ToLower(v["METRIC"]), "memory") &&
					strings.Contains(v["VALUE"], "3.90") &&
					strings.Contains(strings.ToUpper(v["VERDICT"]), "REJECTED")
			},
		},
	}
