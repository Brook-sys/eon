...
func main() {
	...
	outDir := filepath.Join("results", "phase401-quoted-key-parser")
	...
	cases := []struct {
		...
		{
			name:         "double_quoted_keys",
			instruction:  "Extract database configuration facts. Format response strictly as:\n\"DB_NAME\": postgres\n\"DB_PORT\": 5432\n\"DB_USER\": admin",
			expectedKeys: []string{"DB_NAME", "DB_PORT", "DB_USER"},
			expectedVals: map[string]string{"DB_NAME": "postgres", "DB_PORT": "5432", "DB_USER": "admin"},
			effort:       "",
			maxTokens:    256,
		},
		{
			name:         "backtick_quoted_keys",
			instruction:  "Extract deployment parameters. Format response strictly as:\n`CLUSTER_ID`: cl-882\n`REGION`: us-east-1\n`REPLICAS`: 3",
			expectedKeys: []string{"CLUSTER_ID", "REGION", "REPLICAS"},
			expectedVals: map[string]string{"CLUSTER_ID": "cl-882", "REGION": "us-east-1", "REPLICAS": "3"},
			effort:       "",
			maxTokens:    256,
		},
		{
			name:         "bulleted_single_quoted_keys",
			instruction:  "Extract release status. Format response strictly as:\n- 'RELEASE_TAG': v1.4.0\n* 'HEALTH': GREEN\n1. 'PROMOTED': YES",
			expectedKeys: []string{"RELEASE_TAG", "HEALTH", "PROMOTED"},
			expectedVals: map[string]string{"RELEASE_TAG": "v1.4.0", "HEALTH": "GREEN", "PROMOTED": "YES"},
			effort:       "",
			maxTokens:    256,
		},
	}
	...
	reportMarkdown := fmt.Sprintf(`# Phase 401 — Quoted Key Parser & Multi-Provider Live Fire Campaign Report
