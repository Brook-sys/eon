...
// Phase 409: Bare key next-line recovery + mixed format adversarial campaign.
// Tests the new bare key detection feature where models emit key names on one
// line and values on the next, alongside improved prompts for the
// next-line value separation scenario and cross-provider validation.
...
func main() {
	...
	outDir := filepath.Join("results", "phase409-barekey-recovery")
	...
	testCases := []TestCase{
		{
			name:         "bare_key_next_line",
			expectedKeys: []string{"DATE", "SOURCE", "STATUS"},
			instruction:  "Restate each field below on its own line, using the format KEY: VALUE. Data: The date is 2026-08-09, the source is Audit Log Beta, and the status is PENDING.",
			effort:       "",
		...
		{
			name:         "bare_key_value_isolation",
			expectedKeys: []string{"EVENT", "SEVERITY", "ACTION"},
			instruction:  "Extract and label the following incident data. Data: An error occurred in the payment gateway at 14:32 UTC. Severity was HIGH. The automated response was to retry 3 times then alert on-call.",
			effort:       "",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(strings.ToLower(v["EVENT"]), "payment") &&
					strings.Contains(strings.ToUpper(v["SEVERITY"]), "HIGH") &&
					(strings.Contains(strings.ToLower(v["ACTION"]), "retry") || strings.Contains(strings.ToLower(v["ACTION"]), "alert"))
			},
		},
		{
			name:         "mixed_format_adversarial",
			expectedKeys: []string{"DATE", "SOURCE", "STATUS", "IMPACT"},
			instruction:  "Extract and restate each labeled field from the following audit record. The record contains 4 fields. Be precise with exact values.\n\nRecord: On 2026-08-09, source system \"Audit Log Beta\" reported status PENDING with impact level LOW.",
			effort:       "",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(v["DATE"], "2026-08-09") &&
					strings.Contains(strings.ToLower(v["SOURCE"]), "audit log") &&
					strings.Contains(strings.ToUpper(v["STATUS"]), "PENDING") &&
					strings.Contains(strings.ToUpper(v["IMPACT"]), "LOW")
			},
		},
	}
