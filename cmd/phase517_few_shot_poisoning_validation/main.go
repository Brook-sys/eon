...
func main() {
	...
	poisonedFormatExample := "date => 2024-01-01"
	...
	for _, m := range models {
		...
		if err != nil {
			...
			res["text"] = resp.Text
			res["success"] = strings.TrimSpace(resp.Text) == "DATE: 2024-08-14"
		}
		results = append(results, res)
		time.Sleep(1 * time.Second)
	}

	resultsDir := filepath.Join("/home/node/.openclaw/workspace/motor-autonomo/results", "phase517_few_shot_poisoning_validation")
	os.MkdirAll(resultsDir, 0755)

	b, _ := json.MarshalIndent(results, "", "  ")
	os.WriteFile(filepath.Join(resultsDir, "results.json"), b, 0644)
	fmt.Println("Phase 517 Done.")
}
