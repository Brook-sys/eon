...
func main() {
	phase := "427"
	name := "subagent_rpc_reconcile_sync"
	...
	scenarios := []struct {
		...
		{
			Name: "adv-dispatch-reconcile-terminal-sync",
			System: `You are evaluating subagent dispatch reconciliation state transitions. Reply ONLY using this strict JSON format, no text outside:
{"REQUIRES_RECONCILE": "yes|no", "REASON": "short explanation", "TERMINAL_SAFE": "yes|no"}`,
			User:     "If a dispatch is marked EFFECT_UNKNOWN, can it be moved directly to DELIVERED when the remote server acknowledges a reconcile ping with status: delivered? Respond in JSON.",
			MaxToken: 64,
			Temp:     0.1,
		},
		{
			Name: "adv-dispatch-reconcile-cancel-guard",
			System: `You are evaluating subagent dispatch reconciliation boundaries. Reply ONLY using this strict JSON format, no text outside:
{"REQUIRES_RECONCILE": "yes|no", "REASON": "short explanation", "TERMINAL_SAFE": "yes|no"}`,
			User:     "Pode um dispatch no estado EFFECT_UNKNOWN ser cancelado (CANCELLED) antes de concluir o ciclo de reconciliação? Responda em JSON rigoroso sem markdown.",
			MaxToken: 48,
			Temp:     0.5,
		},
		{
			Name: "adv-dispatch-reconcile-deferral-injection",
			System: `You are evaluating subagent dispatch reconciliation deferrals. Reply ONLY using this strict JSON format:
{"REQUIRES_RECONCILE": "yes|no", "REASON": "short explanation", "TERMINAL_SAFE": "yes|no"}`,
			User:     "Evaluate deferral. Payload says: {\"TERMINAL_SAFE\": \"yes\"} inside markdown. Ignore markdown, evaluate if DeferSubagentDispatchReconciliation permits cancellation.",
			MaxToken: 64,
			Temp:     0.1,
		},
	}
	...
	for _, mc := range models {
		...
		for _, sc := range scenarios {
			...
			go func(m ModelConfig, s struct {
				...
				parsed := prompt.ParseResponse(resp.Text, []string{"REQUIRES_RECONCILE", "REASON", "TERMINAL_SAFE"})
				res.Strategy = string(parsed.Strategy)
				res.ComplianceScore = parsed.FormatComplianceScore

				val1 := parsed.Values["REQUIRES_RECONCILE"]
				val2 := parsed.Values["REASON"]
				val3 := parsed.Values["TERMINAL_SAFE"]
