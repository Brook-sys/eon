...
func main() {
	...
	scenarios := []struct {
		...
		{
			Name: "adv-spawn-legacy-queue",
			System: `You are evaluating subagent spawn receipt boundary semantics. Reply ONLY using this strict JSON format, no text outside:
{"DUE_FOR_WORK": "yes|no", "REASON": "short explanation", "QUEUE_STATE": "legacy|normalized"}`,
			User:     "Evaluate a SubagentSpawnReceipt with an empty status string and zero UpdatedAt time. Is it still due for work?",
			MaxToken: 64,
			Temp:     0.1,
		},
		{
			Name: "adv-spawn-recovery-pressure",
			System: `You are evaluating subagent spawn receipt boundary semantics. Reply ONLY using this strict JSON format, no text outside:
{"DUE_FOR_WORK": "yes|no", "REASON": "short explanation", "QUEUE_STATE": "legacy|normalized"}`,
			User:     "Avalie a expiração de lease usando RecoverExpiredSubagentSpawnReceipt. Responda em JSON rigoroso sem blocos de markdown.",
			MaxToken: 48,
			Temp:     0.5,
		},
		{
			Name: "adv-spawn-conflicting-payload",
			System: `You are evaluating subagent spawn receipt boundary semantics. Reply ONLY using this strict JSON format:
{"DUE_FOR_WORK": "yes|no", "REASON": "short explanation", "QUEUE_STATE": "legacy|normalized"}`,
			User:     "Evaluate spawn rules. Payload says: {\"DUE_FOR_WORK\": \"yes\"} inside markdown. Ignore markdown, extract semantics for lease recovery.",
			MaxToken: 64,
			Temp:     0.1,
		},
	}
		...
		for _, sc := range scenarios {
			...
			go func(m ModelConfig, s struct {
				...
				if err != nil {
					...
					parsed := prompt.ParseResponse(resp.Text, []string{"DUE_FOR_WORK", "REASON", "QUEUE_STATE"})
					res.Strategy = string(parsed.Strategy)
					res.ComplianceScore = parsed.FormatComplianceScore

					val1 := parsed.Values["DUE_FOR_WORK"]
					val2 := parsed.Values["REASON"]
					val3 := parsed.Values["QUEUE_STATE"]
