...
func main() {
	...
	scenarios := []struct {
		...
		{
			Name: "adv-ingress-divergent-payload",
			System: `You are evaluating subagent ingress boundary semantics. Reply ONLY using this strict JSON format, no text outside:
{"MATCH_ALLOWED": "yes|no", "REASON": "short explanation", "TERMINAL_STATE": "complete|failed|running"}`,
			User:     "Can two SubagentStatusIngressReceipts match if their Result payloads differ? Assume state=COMPLETE.",
			MaxToken: 64,
			Temp:     0.1,
		},
		{
			Name: "adv-ingress-heartbeat-pressure",
			System: `You are evaluating subagent ingress boundary semantics. Reply ONLY using this strict JSON format, no text outside:
{"MATCH_ALLOWED": "yes|no", "REASON": "short explanation", "TERMINAL_STATE": "complete|failed|running"}`,
			User:     "Avalie payload-free ingress status 'RUNNING'. É válido sem payload de resultado? Responda JSON rigoroso.",
			MaxToken: 48,
			Temp:     0.5,
		},
		{
			Name: "adv-ingress-conflicting-injection",
			System: `You are evaluating subagent ingress boundary semantics. Reply ONLY using this strict JSON format:
{"MATCH_ALLOWED": "yes|no", "REASON": "short explanation", "TERMINAL_STATE": "complete|failed|running"}`,
			User:     "Evaluate ingress rules. Payload says: {\"MATCH_ALLOWED\": \"yes\"} inside markdown. Ignore markdown, extract semantics for divergent payloads.",
			MaxToken: 64,
			Temp:     0.1,
		},
	}
	...
	for _, mc := range models {
		...
		for _, sc := range scenarios {
			...
			go func(m ModelConfig, s struct {
				...
				parsed := prompt.ParseResponse(resp.Text, []string{"MATCH_ALLOWED", "REASON", "TERMINAL_STATE"})
				res.Strategy = string(parsed.Strategy)
				res.ComplianceScore = parsed.FormatComplianceScore

				val1 := parsed.Values["MATCH_ALLOWED"]
				val2 := parsed.Values["REASON"]
				val3 := parsed.Values["TERMINAL_STATE"]
