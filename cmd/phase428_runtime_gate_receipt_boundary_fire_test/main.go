...
func main() {
	phase := "428"
	name := "runtime_gate_receipt_boundary"
	...
	scenarios := []struct {
		...
		{
			Name:     "adv-receipt-structural",
			System:   "You are an OpenClaw model executor architect. Return strict JSON.",
			User:     "To avoid losing the record of a provider completion due to a crash, when must the completion receipt be appended to SQLite relative to releasing the in-flight accounting permit? Reply ONLY with JSON: {\"RECEIPT_COMMITTED_FIRST\": \"true|false\", \"REASON\": \"short text\", \"CRASH_SAFE\": \"true|false\"}",
			MaxToken: 64,
			Temp:     0.0,
		},
		{
			Name:     "adv-receipt-cancellation-ptbr",
			System:   "Responda em PT-BR sobre o ModelExecutor. Retorne JSON estrito.",
			User:     "Se o context.Context for cancelado enquanto o modelo responde 200 OK com os tokens, o recibo deve ser gravado ANTES de soltar o semáforo? (Isso evita vazamento ou perda?). Reply ONLY with JSON: {\"RECEIPT_COMMITTED_FIRST\": \"true|false\", \"REASON\": \"short text\", \"CRASH_SAFE\": \"true|false\"}",
			MaxToken: 48,
			Temp:     0.0,
		},
		{
			Name:     "adv-release-before-receipt-structural",
			System:   "You are an OpenClaw model executor architect. Analyze this workflow. Return strict JSON.",
			User:     "If the ModelExecutor releases the in-flight token permit FIRST and then attempts to append the SQLite receipt, is this crash safe? Reply ONLY with JSON: {\"RECEIPT_COMMITTED_FIRST\": \"true|false\", \"REASON\": \"short text\", \"CRASH_SAFE\": \"true|false\"}",
			MaxToken: 64,
			Temp:     0.0,
		},
	}
		...
		for _, sc := range scenarios {
			...
			go func(m ModelConfig, s struct {
				...
				if err != nil {
					...
					parsed := prompt.ParseResponse(resp.Text, []string{"RECEIPT_COMMITTED_FIRST", "REASON", "CRASH_SAFE"})

					res.Strategy = string(parsed.Strategy)
					res.ComplianceScore = parsed.FormatComplianceScore

					val1 := parsed.Values["RECEIPT_COMMITTED_FIRST"]
					val2 := parsed.Values["CRASH_SAFE"]

					if val1 != "" && val2 != "" {
						if s.Name == "adv-release-before-receipt-structural" {
