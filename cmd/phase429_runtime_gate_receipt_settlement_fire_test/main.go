...
func main() {
	phase := "429"
	name := "runtime_gate_receipt_settlement"
	...
	scenarios := []struct {
		...
		{
			Name:     "adv-settlement-idempotent-structural",
			System:   "You are an OpenClaw model executor architect. Return strict JSON.",
			User:     "If a completion receipt is replayed after a crash, the settlement marker ensures the accounting permit is not released twice. Must this marker and the permit release be executed in a single atomic transaction? Reply ONLY with JSON: {\"IS_IDEMPOTENT\": \"true|false\", \"REASON\": \"short text\", \"CRASH_SAFE\": \"true|false\"}",
			MaxToken: 64,
			Temp:     0.0,
		},
		{
			Name:     "adv-settlement-idempotent-ptbr",
			System:   "Responda em PT-BR sobre o ModelExecutor. Retorne JSON estrito.",
			User:     "Se o reconciliador rodar duas vezes sobre o mesmo recibo de completion, a marcação de 'settled' evita estorno duplicado da cota de tokens? Reply ONLY with JSON: {\"IS_IDEMPOTENT\": \"true|false\", \"REASON\": \"short text\", \"CRASH_SAFE\": \"true|false\"}",
			MaxToken: 48,
			Temp:     0.0,
		},
		{
			Name:     "adv-settlement-split-structural",
			System:   "You are an OpenClaw model executor architect. Analyze this workflow. Return strict JSON.",
			User:     "If the ModelExecutor releases the in-flight token permit in one transaction, and then marks the receipt as 'settled' in a separate second transaction, is this crash safe against duplicate releases during recovery? Reply ONLY with JSON: {\"IS_IDEMPOTENT\": \"true|false\", \"REASON\": \"short text\", \"CRASH_SAFE\": \"true|false\"}",
			MaxToken: 64,
			Temp:     0.0,
		},
	}
		...
		for _, sc := range scenarios {
			...
			go func(m ModelConfig, s struct {
				...
				if err != nil {
					...
					parsed := prompt.ParseResponse(resp.Text, []string{"IS_IDEMPOTENT", "REASON", "CRASH_SAFE"})

					res.Strategy = string(parsed.Strategy)
					res.ComplianceScore = parsed.FormatComplianceScore

					val1 := parsed.Values["IS_IDEMPOTENT"]
					val2 := parsed.Values["CRASH_SAFE"]

					if val1 != "" && val2 != "" {
						if s.Name == "adv-settlement-split-structural" {
							if (val1 == "no" || val1 == "false") && (val2 == "no" || val2 == "false") {
								res.Success = true
							}
						} else {
							if (val1 == "yes" || val1 == "true") && (val2 == "yes" || val2 == "true") {
								res.Success = true
							}
						}
					}
