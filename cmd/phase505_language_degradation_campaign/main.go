...
func main() {
	...
	// Mixed PT-BR / EN language in facts
	degradedPayload := `A data of the operation was realized no dia 15 de março de 2024. The origem do evento was the sistema central de faturamento. Source details follow.`
	...
	input := prompt.Input{
		...
		Facts:                 []prompt.Fact{{ID: "F1", Text: degradedPayload, Required: true}},
	...
	for _, m := range models {
		...
		if err != nil {
			...
			res["text"] = resp.Text
			res["success"] = strings.Contains(resp.Text, "2024-03-15") && strings.Contains(strings.ToLower(resp.Text), "faturamento")
		}
		results = append(results, res)
		time.Sleep(1 * time.Second)
	}

	resultsDir := filepath.Join("results", "phase505_language_degradation_campaign")
	os.MkdirAll(resultsDir, 0755)

	b, _ := json.MarshalIndent(results, "", "  ")
	os.WriteFile(filepath.Join(resultsDir, "results.json"), b, 0644)
	fmt.Println("Phase 505 Done.")
}
