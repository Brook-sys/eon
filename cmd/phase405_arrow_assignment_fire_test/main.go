...
func main() {
	log.Println("=== Phase 405: Arrow & Assignment Separator Parsing Live Fire Campaign ===")
	...
	outDir := filepath.Join("results", "phase405-arrow-assignment-parser")
	...
	testCases := []TestCase{
		{
			name:         "arrow_separators",
			expectedKeys: []string{"DATE", "SOURCE", "STATUS"},
			instruction:  "Extract report details (Date is 2026-08-08, Source is SecurityAudit, Status is CONFIRMED). Use ASCII arrows as separators e.g. DATE -> 2026-08-08, SOURCE => SecurityAudit, STATUS -> CONFIRMED.",
		...
		{
			name:         "assignment_separators",
			expectedKeys: []string{"ALERT_ID", "SEVERITY"},
			instruction:  "Process alert details (Alert ID is ALT-9921, Severity is CRITICAL). Use assignment or double colon separators e.g. ALERT_ID := ALT-9921, SEVERITY :: CRITICAL.",
			checkSem: func(v map[string]string) bool {
		...
		{
			name:         "bulleted_arrow_mix",
			expectedKeys: []string{"BUILD", "ERRORS", "TARGET"},
			instruction:  "Build status update (Build is PASS, Errors count is 0, Target platform is Linux-x64). Format output as bulleted items with arrows e.g. - BUILD => PASS, * ERRORS -> 0, 1. TARGET => Linux-x64.",
			checkSem: func(v map[string]string) bool {
	...
	log.Printf("Phase 405 completed: %d/%d success (%.1f%%), P50 latency: %dms, P95 latency: %dms",
...
func writeReport(path string, s CampaignSummary, trials []Trial) {
	...
	sb.WriteString("# Phase 405 Live Fire Campaign Report: Arrow & Assignment Separators\n\n")
