...
func main() {
	log.Println("=== Phase 404: Bracket Key & Alternate Separator Parsing Live Fire Campaign ===")
	...
	outDir := filepath.Join("results", "phase404-bracket-separator-parser")
	...
	testCases := []TestCase{
		{
			...
			instruction:  "Extract report details (Date is 2026-08-08, Source is SecurityAudit, Status is CONFIRMED). Enclose key names in brackets e.g. [DATE]: 2026-08-08, (SOURCE): SecurityAudit, [STATUS]: CONFIRMED.",
		...
		{
			name:         "dash_separators",
			expectedKeys: []string{"ALERT_ID", "SEVERITY"},
			instruction:  "Process alert details (Alert ID is ALT-9921, Severity is CRITICAL). Format output using dashes or equals sign as separator instead of colon e.g. ALERT_ID - ALT-9921, SEVERITY = CRITICAL.",
			checkSem: func(v map[string]string) bool {
		...
		{
			name:         "bracket_tags_and_headers",
			expectedKeys: []string{"BUILD", "ERRORS", "TARGET"},
			instruction:  "Build status update (Build is PASS, Errors count is 0, Target platform is Linux-x64). Format output as bracketed numbered lines e.g. [1] [BUILD]: PASS, [2] (ERRORS) - 0, [3] 【TARGET】 = Linux-x64.",
			checkSem: func(v map[string]string) bool {
	...
	log.Printf("Phase 404 completed: %d/%d success (%.1f%%), P50 latency: %dms, P95 latency: %dms",
...
func writeReport(path string, s CampaignSummary, trials []Trial) {
	...
	sb.WriteString("# Phase 404 Live Fire Campaign Report: Bracket Keys & Alternate Separators\n\n")
