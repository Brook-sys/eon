...
func main() {
	...
	maxTokensList := []int{256, 1024, 2048}
	cases := []struct {
		effort string
		format string
	}{
	...
	var results []result
	for _, maxTok := range maxTokensList {
		for _, tc := range cases {
			fmt.Fprintf(os.Stderr, "Testing qwen3.6-27b max_tokens=%d effort=%q format=%q ...\n", maxTok, tc.effort, tc.format)
