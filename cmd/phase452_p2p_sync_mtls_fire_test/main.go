...
func main() {
	...
	fmt.Println("=== Phase 452: P2P Sync Capability over mTLS Transport Live Fire Test ===")

	// 1. Setup PKI
	tmpDir, err := os.MkdirTemp("", "p2p-phase452-*")
	if err != nil {
	...
	caTemplate := x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "phase452-ca"},
		NotBefore:             time.Now().Add(-1 * time.Minute),
		NotAfter:              time.Now().Add(1 * time.Hour),
	...
	syncCap := peersync.Capability
	fmt.Printf("P2P Managers A and B started with mTLS transport. Sync Capability %q registered.\n", syncCap)
	...
	req := port.CompletionRequest{
		Prompt: `Evaluate the following P2P state sync component:
- Event Sync capability "event.sync.v1" registered on P2P Router.
- State replication messages authenticated over mutual TLS (mTLS).
- Durable event log inbox and outbound cursor tracking enabled.

Determine if P2P state sync over mTLS is active and verified.
...
CONFLICT_RESOLVER: ACTIVE`,
	...
	outDir := filepath.Join("results", "phase452-p2p_sync_mtls_fire_test")
