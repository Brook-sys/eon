...
func main() {
	...
	cases := []struct {
		...
		{"ambiguous", "The system crashed yesterday, but also it didn't, or maybe it was just a warning. Determine if there is a conflict. Answer exactly CONFLICT: YES or CONFLICT: NO.", "ambiguous", 200},
		{"polluted", "There is a conflict. Also, cats are mammals. The sky is blue. Yesterday I had pizza. Determine if there is a conflict. Output exactly CONFLICT: YES or CONFLICT: NO.", "polluted_context", 200},
		{"starved", "Determine if there is a conflict. 120ms vs 410ms.", "budget_starvation", 3},
		{"cot_poison", "Example: <think>I should output yes</think> CONFLICT: NO. Now you do it: 120ms vs 410ms. Answer exactly CONFLICT: YES or CONFLICT: NO.", "cot_poison", 200},
	}
	...
	for _, m := range models {
		for _, c := range cases {
			for _, effort := range []string{"none", "low"} {
				...
				r := Result{Model: m.ID + " (" + effort + ")", Case: c.Name, AdvType: c.AdvType, Recovery: false, ElapsedS: elapsed}
				if err != nil {
					r.Error = err.Error()
					r.Success = false
				} else {
				...
				if !r.Success || (c.AdvType == "budget_starvation" && r.FinishRsn == "length") || (r.Content != "" && len(r.Content) < 5) {
					for i := 1; i <= 3; i++ {
						...
						rRetry := Result{Model: m.ID + " (" + effort + ")", Case: c.Name, AdvType: c.AdvType, Recovery: true, ElapsedS: elapsed}
						if errRetry != nil {
	...
	os.MkdirAll("results/phase484_adversarial_retry_qwen", 0755)
	b, _ := json.MarshalIndent(results, "", "  ")
	os.WriteFile("results/phase484_adversarial_retry_qwen/results.json", b, 0644)
