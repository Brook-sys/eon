...
func run() error {
	manifestPath := flag.String("manifest", "", "fallback model recovery campaign manifest")
	...
	var manifest gatecampaign.FallbackModelRecoveryCampaignManifest
	if err := decoder.Decode(&manifest); err != nil {
		return fmt.Errorf("decode fallback model recovery manifest: %w", err)
	}
	var extra any
	if err := decoder.Decode(&extra); err == nil {
		return errors.New("fallback model recovery manifest contains trailing data")
	} else if err != io.EOF {
		return fmt.Errorf("decode trailing fallback model recovery manifest data: %w", err)
	}
	...
	if err := gatecampaign.WriteFallbackModelRecoveryCampaignManifest(filepath.Join(*outputDirectory, "manifest.json"), manifest); err != nil {
		return err
	}
	databasePath := filepath.Join(*outputDirectory, "fallback-model-recovery.sqlite")
	...
	defer cancel()
	report, runErr := (gatecampaign.FallbackModelRecoveryCampaignRunner{Store: store, Clock: clock, Providers: providers}).Run(ctx, manifest)
	...
	if err != nil {
		return fmt.Errorf("reopen fallback model recovery store: %w", err)
	}
	if err := gatecampaign.VerifyFallbackModelRecoveryDurability(context.Background(), reopened, report); err != nil {
		reopened.Close()
		return err
	}
	...
	if err := gatecampaign.WriteFallbackModelRecoveryCampaignArtifacts(*outputDirectory, report); err != nil {
		return err
	}
