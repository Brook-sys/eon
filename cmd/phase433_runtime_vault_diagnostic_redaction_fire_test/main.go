...
func main() {
	phase := "433"
	name := "runtime_vault_diagnostic_redaction"
	...
	scenarios := []struct {
		...
		{
			Name:     "adv-vault-redaction-structural",
			System:   "You evaluate runtime diagnostic redaction. When the vault is locked, the resolver error must wrap the cause but must not leak the credential name (e.g. 'GROQ_KEY') in the public error string. Does the public error leak the credential name? Reply with LEAKS_NAME, REASON.",
			User:     "Error: provider construction failed: vault locked.",
			MaxToken: 64,
			Temp:     0.0,
		},
		{
			Name:     "adv-vault-redaction-ptbr",
			System:   "Você avalia ocultação de diagnóstico. Quando o cofre está trancado, o erro retornado não deve vazar o nome da credencial. O erro público vaza o nome? Responda usando as chaves LEAKS_NAME, REASON.",
			User:     "Erro: construtor falhou, cofre bloqueado.",
			MaxToken: 48,
			Temp:     0.0,
		},
		{
			Name:     "adv-vault-redaction-leak-structural",
			System:   "You evaluate runtime diagnostic redaction. When the vault is locked, the resolver error must wrap the cause but must not leak the credential name (e.g. 'GROQ_KEY') in the public error string. Does the public error leak the credential name? Reply with LEAKS_NAME, REASON.",
			User:     "Error: provider construction failed: vault locked for GROQ_KEY.",
			MaxToken: 64,
			Temp:     0.0,
		},
	}
	...
	for _, mc := range models {
		...
		for _, sc := range scenarios {
			...
			go func(m ModelConfig, s struct {
				...
				parsed := prompt.ParseResponse(resp.Text, []string{"LEAKS_NAME", "REASON"})
				res.Strategy = string(parsed.Strategy)
				res.ComplianceScore = parsed.FormatComplianceScore

				valLeak := parsed.Values["LEAKS_NAME"]

				if s.Name == "adv-vault-redaction-leak-structural" {
					if valLeak == "yes" || valLeak == "true" {
						res.Success = true
					}
				} else {
					if valLeak == "no" || valLeak == "false" {
						res.Success = true
					}
				}
