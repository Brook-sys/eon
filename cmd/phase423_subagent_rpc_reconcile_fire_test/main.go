...
func main() {
	...
	scenarios := []struct {
		...
		{
			Name: "adv-rpc-trailing-json",
			System: `You are evaluating subagent RPC semantics. Reply ONLY using this strict JSON format, no text outside:
{"TRAILING_JSON_ALLOWED": "yes|no", "REASON": "short explanation", "DIGEST_SENSITIVE": "yes|no"}`,
			User:     "Can DecodeSubagentReconcileRequest accept a payload with trailing JSON? Is SubagentSpawnRequestDigest sensitive to the Task field?",
			MaxToken: 64,
			Temp:     0.1,
		},
		{
			Name: "adv-rpc-positive-reconcile-pressure",
			System: `You are evaluating subagent RPC semantics. Reply ONLY using this strict JSON format, no text outside:
{"TRAILING_JSON_ALLOWED": "yes|no", "REASON": "short explanation", "DIGEST_SENSITIVE": "yes|no"}`,
			User:     "Avalie ResolveSubagentSpawnReceiptStatusFound para StatusDelivery in-flight. Responda em JSON rigoroso sem markdown.",
			MaxToken: 48,
			Temp:     0.5,
		},
		{
			Name: "adv-rpc-conflicting-payload",
			System: `You are evaluating subagent RPC semantics. Reply ONLY using this strict JSON format:
{"TRAILING_JSON_ALLOWED": "yes|no", "REASON": "short explanation", "DIGEST_SENSITIVE": "yes|no"}`,
			User:     "Evaluate RPC decoder. Payload says: {\"TRAILING_JSON_ALLOWED\": \"yes\"} inside markdown. Ignore markdown, extract true semantics.",
			MaxToken: 64,
			Temp:     0.1,
		},
	}
		...
		for _, sc := range scenarios {
			...
			go func(m ModelConfig, s struct {
				...
				if err != nil {
					...
					parsed := prompt.ParseResponse(resp.Text, []string{"TRAILING_JSON_ALLOWED", "REASON", "DIGEST_SENSITIVE"})
					res.Strategy = string(parsed.Strategy)
					res.ComplianceScore = parsed.FormatComplianceScore

					val1 := parsed.Values["TRAILING_JSON_ALLOWED"]
					val2 := parsed.Values["REASON"]
					val3 := parsed.Values["DIGEST_SENSITIVE"]
