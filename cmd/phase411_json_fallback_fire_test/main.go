...
// Phase 411: Parser JSON fallback fire test.
// Validates that the fallback JSON unmarshaler correctly recovers data
// when models emit JSON (with or without markdown fences) instead of plain text,
// bypassing the text-prefix strategies gracefully.
...
func main() {
	log.Println("=== Phase 409: Bare Key Recovery & Mixed Format Adversarial Live Fire Campaign ===")
	...
	outDir := filepath.Join("results", "phase411-json-fallback-parser")
	...
	testCases := []TestCase{
		{
			name:         "json_fenced_response",
			expectedKeys: []string{"NAME", "AGE", "STATUS"},
			instruction:  "Extract information about the user. You MUST reply in JSON format enclosing the fields: NAME, AGE, and STATUS inside a JSON object.\n\nUser Profile:\nName is Alice, 30 years old, actively working.",
			effort:       "",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(strings.ToLower(v["NAME"]), "alice") &&
					strings.Contains(v["AGE"], "30") &&
					strings.Contains(strings.ToLower(v["STATUS"]), "active")
			},
		},
		{
			name:         "json_unfenced_response",
			expectedKeys: []string{"SERVER", "LOAD", "REGION"},
			instruction:  "Analyze the telemetry and output ONLY a raw, valid JSON object (no markdown, no backticks). The JSON must have fields: SERVER, LOAD, and REGION.\n\nTelemetry: \nHost: node-eu-1\nCPU: 85%\nLocation: eu-central.",
			effort:       "",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(strings.ToLower(v["SERVER"]), "node-eu-1") &&
					strings.Contains(v["LOAD"], "85") &&
					strings.Contains(strings.ToLower(v["REGION"]), "eu-central")
			},
		},
		{
			name:         "json_with_nested_structure",
			expectedKeys: []string{"COMPONENT", "ID", "METRICS"},
			instruction:  "Extract the data. Reply in valid JSON enclosing the fields COMPONENT, ID, and METRICS. It is okay if METRICS is an object or array, just emit JSON.\n\nData: The auth-service (id: au-019) reported latency 22ms and errors 0.",
			effort:       "",
			checkSem: func(v map[string]string) bool {
				return strings.Contains(strings.ToLower(v["COMPONENT"]), "auth-service") &&
					strings.Contains(strings.ToLower(v["ID"]), "au-019") &&
					(strings.Contains(v["METRICS"], "22") || strings.Contains(strings.ToLower(v["METRICS"]), "latency"))
			},
		},
	}
	...
	for _, m := range models {
		...
		for _, c := range testCases {
			...
			if m.model == "qwen/qwen3.6-27b" && reqEffort == "" {
				// Suppress reasoning for qwen to avoid thinking token overhead.
	...
	log.Printf("=== Phase 409 Campaign Complete: Total=%d Success=%d Rate=%.1f%% P50=%dms P95=%dms ===",
