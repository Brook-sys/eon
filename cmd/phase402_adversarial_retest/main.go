...
func main() {
	...
	outDir := filepath.Join("results", "phase402-adversarial-retest")
	...
	// Hardest adversarial tasks from Aug 5 sweep
	cases := []struct {
		name         string
		instruction  string
		expectedKeys []string
	...
	for _, m := range models {
		...
		for _, c := range cases {
			...
			spec := domain.OperationSpec{
				SchemaVersion:    1,
				ID:               "adversarial_retest@1",
				ContractVersion:  1,
				TemplateVersion:  1,
				InputSchema:      "adversarial_facts",
	...
	_ = os.WriteFile(filepath.Join(outDir, "summary.json"), summaryBytes, 0644)

	trialsBytes, _ := json.MarshalIndent(trials, "", "  ")
	_ = os.WriteFile(filepath.Join(outDir, "trials.json"), trialsBytes, 0644)

	// Write markdown report
	reportPath := filepath.Join(outDir, "REPORT.md")
	var report strings.Builder
	report.WriteString("# Phase 402 — Adversarial Retest & Hardened Go Parser Campaign Report\n\n")
	report.WriteString(fmt.Sprintf("**Date:** %s\n", time.Now().UTC().Format(time.RFC3339)))
	report.WriteString(fmt.Sprintf("**Total Trials:** %d\n", summary.TotalTrials))
	report.WriteString(fmt.Sprintf("**Success Count:** %d (%.1f%%)\n", summary.SuccessCount, summary.SuccessRate*100))
	report.WriteString(fmt.Sprintf("**P50 Latency:** %d ms\n", summary.P50LatencyMS))
	report.WriteString(fmt.Sprintf("**P95 Latency:** %d ms\n", summary.P95LatencyMS))
	report.WriteString(fmt.Sprintf("**Average Compliance Score:** %.2f\n\n", summary.ComplianceAvg))

	report.WriteString("## Strategy Distribution\n")
	for strat, count := range summary.StrategyCounts {
		report.WriteString(fmt.Sprintf("- **%s**: %d\n", strat, count))
	}
	report.WriteString("\n## Trials Breakdown\n\n")
	report.WriteString("| Provider | Model | Case | MaxTokens | Retried | Strategy | Compliance | Semantic | Latency |\n")
	report.WriteString("|---|---|---|---|---|---|---|---|---|\n")
	for _, t := range trials {
		report.WriteString(fmt.Sprintf("| %s | %s | %s | %d | %v | %s | %.2f | %v | %dms |\n",
	...
	fmt.Printf("\n=== Phase 402 Campaign Complete ===\n")
	fmt.Printf("Total: %d | Success: %d (%.1f%%) | P50: %dms | P95: %dms | ComplianceAvg: %.2f\n",
		summary.TotalTrials, summary.SuccessCount, summary.SuccessRate*100, summary.P50LatencyMS, summary.P95LatencyMS, summary.ComplianceAvg)
}
