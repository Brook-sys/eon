...
func main() {
	phase := "434"
	name := "runtime_executor_reload_preservation"
	...
	scenarios := []struct {
		...
		{
			Name:     "adv-reload-preservation-structural",
			System:   "You evaluate executor reload boundaries. When hot reload fails during provider assembly, the system must retain the prior active executor, not clear it. Does a failed reload retain the prior executor? Reply with RETAINS_PRIOR, CLEARS_ON_FAIL, REASON.",
			User:     "Reload fails because vault is locked. Prior executor is active.",
			MaxToken: 64,
			Temp:     0.0,
		},
		{
			Name:     "adv-reload-preservation-ptbr",
			System:   "Você avalia recarregamento. Se a montagem do provedor falhar, o sistema mantém o executor anterior e não o apaga. O sistema mantém o executor anterior? Responda com RETAINS_PRIOR, CLEARS_ON_FAIL, REASON.",
			User:     "O recarregamento falhou. O executor anterior continua ativo.",
			MaxToken: 48,
			Temp:     0.0,
		},
		{
			Name:     "adv-reload-atomic-structural",
			System:   "You evaluate executor reload boundaries. When hot reload fails during provider assembly, the system must retain the prior active executor, not clear it. Does the failed reload return a partial/broken executor? Reply with RETURNS_PARTIAL, REASON.",
			User:     "Reload fails because vault is locked.",
			MaxToken: 64,
			Temp:     0.0,
		},
	}
